...
class CategoryModel extends BaseModel {
    ...
    {
        if(!empty($info['cid'])){
            $type = $this->getInfo($info['cid']);
            return match_url(strtolower($type['app']).'/Category/index',array('class_id'=>$type['class_id'],'urlname'=>$type['urlname']));
        }elseif(!empty($info['url']) && empty($info['cid'])){
            return $info['url'];
        }else{
