...
function listData(date) {
	...
	$.ajax({
		url: "http://www.900kcw.com/?r=home/count/gxklsf_road_bead&date=" + date,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(rank, data) {
	...
	1-8分别为1-5球,6为总和
	state形态: 
	1：单双和；2：大小和；3：尾大小；4：号码合单双；5：第一球龙虎
	totals：
	单双和、大小和、尾大、尾小
	0：单、大、龙；1：双、小，虎；2：和
	*/
	...
	if((rank != 6) && (data.state == 1 || data.state == 2)){//第1~5球单双，大小
	...
	$(data.roadBeads).each(function(index) {
		...
		if(box == "61") {
			newroadbeads = (data.roadBeads[index] == 1) ? "单" : "双";
			$boxnumb.find(".left_count").text(data.totals[0]);
			$boxnumb.find(".middle_count").text(data.totals[1]);
			$boxnumb.find(".right_count").text(data.totals[2]);
		}
...
function forRankYibu(rank, data) {
	...
	var $middle_count = $boxnumb.find(".middle_count");
