...
function listData() {
	...
	$.ajax({
		url: "http://www.900kcw.com/?r=home/count/klsf_today_num_roadbead",
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	*/
	...
	$(data).each(function(index) {
		var trhtml = "";
		trhtml += "<td>" + this.code + "</td>";
		$(this.list).each(function(index) {
			trhtml += "<td><span>" + Math.abs(this.accumulate) + "</span></td><td><span>" + Math.abs(this.missing) + "</span></td>";
		});
