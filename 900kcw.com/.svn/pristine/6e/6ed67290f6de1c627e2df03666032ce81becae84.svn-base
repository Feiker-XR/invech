	...
	$("#startVideo").on("click", function() {
			...
			$("#videobox").animate({
				...
				$(".content").animate({
					...
					var numArr = [];
					var preDrawIssue = $("#gxklsf").find(".preDrawIssue").text();
					var nextIssue = $("#gxklsf").find(".nextIssue").text();
					var drawTime = $("#gxklsf").find("#drawTime").val();
					...
					$("iframe")[0].contentWindow.gxklsf.startVid(data)
				});
			});
...
function listData(code) {
	$.ajax({
		url: "http://www.900kcw.com/?r=home/history/gxkl10f&=" + code,
		type: "GET",
		// data: {
		// 	"lotCode": lotCode
		// },
...
function todayData() {
	$.ajax({
		url: "http://www.900kcw.com/?r=home/lottery/gxklsf_double_count",
		type: "GET",
		data: {
			"lotCode": 10038
		},
...
function longData() {
	$.ajax({
		url: "http://www.900kcw.com/?r=home/lottery/gxklsf_long_dragon&date=",
		type: "GET",
		data: {
			"lotCode": 10038
		},
...
function loadTodayData(jsondata) {
	...
	$(td).find(".one5").text(data.firstEqualsCount);
	...
	$(td).find(".two5").text(data.secondEqualsCount);
	...
	$(td).find(".three5").text(data.thirdEqualsCount);
	...
	$(td).find(".four5").text(data.fourthEqualsCount);
	...
	$(td).find(".five5").text(data.fifthEqualsCount);
...
function loadLongData(jsondata) {
	...
	for(var i = 0, len = data.length; i < len; i++) {
		...
		if(data[i].rank == 6) {
			html = "<li><span>" + "总和" + "</span>：&nbsp;&nbsp;<span>" + state + "</span>&nbsp;&nbsp;" + counthtml + "期</li>";
		}
		$("#cltxul").append(html);
	}
...
function createHtmlList(jsondata) {
	...
	$("#jrsmhmtj>table").html('<tr><th width=150>时间</th><th width=80>期数</th><th id="numberbtn" class="numberbtn" width=330><span id="xshm" class="spanselect">显示号码</span><span id="xsdx">显示大小</span><span id="xsds">显示单双</span></th><th colspan="3">总和</th><th>尾大小</th><th>龙虎</th></tr>');
	$(data).each(function(index) {
		...
		$(preDrawCode).each(function() {
			if(this == 1 || this == 4 || this == 7|| this == 10 || this == 13|| this == 16 || this == 19) {
				tdli += "<li class='gxnumred' style='color:#292c2e;font-size:18px'><i>" + this + "</i></li>";
			} else if(this == 2 || this == 5 || this == 8 || this == 11 || this == 14 || this == 17 || this == 20) {
				tdli += "<li class='gxnumblue' style='color:#292c2e;font-size:18px'><i>" + this + "</i></li>";
			} else if(this == 3  || this == 6  || this == 9  || this == 12  || this == 15  || this == 18  || this == 21){
				tdli += "<li class ='gxnumgreen' style='color:#292c2e;font-size:18px'><i>" + this + "</i></li>"
			}
		});
		var stylestr = "style='color:";
		var sumNum = "<td>" + this.sumNum + "</td>";
		if(!(preDrawCode.length <= 1)) {
			var tdnumber = "<td class='blueqiu'><ul class='gx_kaihmul'>" + tdli + "</ul></td>";
			var sumSingleDouble = "";
		...
		var last = "<td " + style1 + ">" + sumSingleDouble + "</td><td " + style2 + ">" + sumBigSmall + 　"</td><td " + style3 + ">" + lastBigSmall + "</td><td " + style4 + ">" + firstDragonTiger + "</td>";
