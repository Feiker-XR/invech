...
//        ini_set('display_errors', 'On');
    ...
    protected function setCont(){
        ...
        $siteConfig['site_url'] = 'www.900kcw.com' ; //临时编辑
        $this->sys = $siteConfig;
        ...
        if(config('mobile_status')){
            //网站跳转
            if (!$detect->isMobile() && !$detect->isTablet() && $url<>config('mobile_domain')){
		...
		if(config('ipad_status')){
            //网站跳转
            if ($url<>config('ipad_domain')){
                if(config('site_url')&&$url<>config('site_url')){
                    $this->redirect('http://'.config('site_url').$_SERVER["REQUEST_URI"]);
                }
                define('IPAD',false);
            }else{
                if(config('ipad_domain')&&$url<>config('ipad_domain')){
                    $this->redirect('http://'.config('ipad_domain').$_SERVER["REQUEST_URI"]);
                }
                define('IPAD',true);
            }
        }else{
		...
		if(config('tv_status')){
            //网站跳转
            if ($url<>config('tv_domain')){
                if(config('site_url')&&$url<>config('site_url')){
                    $this->redirect('http://'.config('site_url').$_SERVER["REQUEST_URI"]);
                }
                define('TV',false);
            }else{
                if(config('tv_domain')&&$url<>config('tv_domain')){
                    $this->redirect('http://'.config('tv_domain').$_SERVER["REQUEST_URI"]);
                }
                define('TV',true);
            }
        }else{
