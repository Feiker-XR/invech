$(function() {
	...
	//今天，昨天，前天选择操作
	$(".listheadrl span").on("click", function() {
	...
	$("#startVideo").on("click", function() {
		...
		document.addEventListener('touchstart', function() {
			...
			$("iframe")[0].contentWindow.phpcument.getElementById("sound").play();
		}, false);
...
function ajaxRequst(issue, id, flag) {
	...
	var iferror = false;
	var code = $('#pk10').find('.nextIssue').text();
	// if ($("#pk10 .cuttime").find(".minute").text() == 0 && $("#pk10 .cuttime").find(".second").text() == 0) {
	// 	code = "";
	// }

	$.ajax({
		// url: urlbublic + "pks/getLotteryPksInfo.php?issue=" + issue,
		url: "http://www.900kcw.com/?r=home/lottery/pk10&num=" + code,
		type: "get",
		timeout: 60000,
		asasync: false,
		// data: {
		...
		success: function(data) {
			try {
				if (flag ==1) {
                    creatDataHead_2(data, id); //向头部填充数据
				} else {
...
//向开奖区填数据
function creatDataHead_2(jsondata, id) {
	var data = tools.parseObj(jsondata);
	data = data.result.data;
	var timeResult = tools.operatorTime(data.drawTime == "" ? "0" : data.drawTime, data.serverTime); //得到时间差
	if(timeResult <= 0) {
		...
		if(!$("#videobox").length < 1) {
			if(!($("#videobox").css("z-index") == -1)) {
				$("iframe")[0].contentWindow.finishgame(data.preDrawCode);
				setTimeout(function() {
					publicmethod.insertHeadPk10_2(jsondata, id);
				}, 3000);
				setTimeout(function() {
					tools.insertVideo(); //切换到开奖
				}, 10000);
