...
function listData() {
	...
	$.ajax({
		url: urlbublic + "lotteryJSFastThree/queryOrientationTrend.php?date=" + time + "&issue=" + issue + "&periods=" + periods,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	$(data).each(function(index) {
		...
		$(this.bodyList).each(function() {
			...
			var missingtd2 = "";
			var missingtd3 = "";
			$(this.oneCode).each(function(index) {
				var title = "";
				var bgqiu = "";
				var value = "";
				if(this * 1 > 0) {
				...
				missingtd1 += "<td " + stylebg + " " + title + "><span " + bgqiu + ">" + value + "</span></td>";
			});
			$(this.twoCode).each(function(index) {
				var title = "";
				var bgqiu = "";
				var value = "";
				if(this * 1 > 0) {
				...
				missingtd2 += "<td " + stylebg + " " + title + "><span " + bgqiu + ">" + value + "</span></td>";
			});
			$(this.threeCode).each(function(index) {
				var title = "";
				var bgqiu = "";
				var value = "";
				if(this * 1 > 0) {
			...
			var tr = "<tr class='yiloutr'>" + preIssue + "" + preDrawCode + "" + missingtd1 + "" + missingtd2 + "" + missingtd3 + "</tr>";
				...
				function() {
					if($(this).attr("title") == "0") {
						if($(this).index() >= 2 && $(this).index() <= 7) {
							...
							$(this).attr("class", "hot " + $(this).removeClass("grey").attr("class")).find("span").attr("name", "hotSpan").text($(this).attr("name")).attr("class", "zoushiqiuf outdbg");
							$(this).find("span").css("background", "#3cb0ec");
						} else if($(this).index() >= 12 && $(this).index() <= 17) {
							$(this).attr("class", "hot " + $(this).removeClass("grey").attr("class")).find("span").attr("name", "hotSpan").text($(this).attr("name")).attr("class", "zoushiqiu bBK");
							$(this).find("span").css("background", color[1]);
							...
							$(this).attr("class", "hot " + $(this).removeClass("grey").attr("class")).find("span").attr("name", "hotSpan").text($(this).attr("name")).attr("class", "zoushiqiuf outdbg");
							$(this).find("span").css("background", "#3cb0ec");
						} else if($(this).index() >= 22 && $(this).index() <= 27) {
							$(this).attr("class", "hot " + $(this).removeClass("grey").attr("class")).find("span").attr("name", "hotSpan").text($(this).attr("name")).attr("class", "zoushiqiu bBK");
							$(this).find("span").css("background", color[1]);
