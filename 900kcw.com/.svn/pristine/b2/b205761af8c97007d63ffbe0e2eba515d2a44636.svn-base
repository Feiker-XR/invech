...
function listData(time) {
	...
	$.ajax({
		url: "http://www.900kcw.com/?r=home/count/k3_trend_byissue&date=" + time + "&issue=" + issue + "&periods=" + periods,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
		...
		$(this.bodyList).each(function() {
			...
			$(drawCode).each(function(index) {
				...
				for(var i = 0; i < attr.length; i++) {
					for(var j = i + 1; j < drawCode.length; j++) {
						if(attr[i] == drawCode[j]) {
							count += 1;
							value1 = attr[i];
						} else {
							return;
			...
			var missingtd23 = "";
			var missingtd3 = "";
			$(this.distribution).each(function(index) {
				var title = "";
				var bgqiu = "";
				var value = "";
				if(this * 1 > 0) {
			...
			$(this.numberForm).each(function(index) {
				var title = "";
				var bgqiu = "";
				var value = "";
				if(index == 0) {
					...
					missingtd23 += "<td " + stylebg + " " + title + "><span " + bgqiu + ">" + value + "</span></td>";
				}
				var stylebg = "";
				if(index < 26) {
					stylebg = "style='background-color:#fcf9f2'"
			...
			$(this.sumNum).each(function(index) {
				...
				missingtd3 += "<td " + stylebg + " " + title + "><span " + bgqiu + ">" + value + "</span></td>";
			});

			var tr = "<tr class='yiloutr'>" + preIssue + "" + preDrawCode + "" + missingtd1 + "" + missingtd21 + "" + missingtd22 + "" + missingtd23 + "" + missingtd3 + "</tr>";
				...
				function(index) {
					if($(this).attr("title") == "0") {
						if($(this).index() >= 2 && $(this).index() <= 7) {
							...
							$(this).attr("class", "hot " + $(this).removeClass("grey").attr("class")).find("span").attr("name", "hotSpan").text($(this).attr("name")).attr("class", "zoushiqiuf jitdbg");
							$(this).find("span").css("background", "#fa9932");
						} else if($(this).index() >= 11 && $(this).index() <= 26) {
