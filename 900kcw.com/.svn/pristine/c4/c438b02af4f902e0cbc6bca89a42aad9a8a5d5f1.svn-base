$(function() {
	...
	$("#startVideo").on("click", function() {
		if($("#pk10num").find("li").length == 8) {
			$("iframe")[0].contentWindow.cqncVideo.soundstatus();
			var datas = createData();
			startVideo();
			$("iframe")[0].contentWindow.ifopen = true;
			if($("iframe")[0].contentWindow.find(".roll_1") != undefined) {
				setTimeout(function() {
					console.log("s_arr:" + datas.codearr);
					$("iframe")[0].contentWindow.cqncVideo.statusFun(datas.preDrawIssue, datas.codearr, datas.counttime, true);
...
method.listData = function(date) {
	$.ajax({
		url: "http://m.900kcw.com/?r=home/history/xync",
		type: "GET",
		// data: {
		// 	"lotCode": lotCode
		// },
...
method.todayData = function(date) {
	$.ajax({
		url: "http://m.900kcw.com/?r=home/lottery/klsf_double_count" + date,
		type: "GET",
		data: {
			"lotCode": lotCode
		},
...
method.longData = function(date) {
	$.ajax({
		url: "http://m.900kcw.com/?r=home/lottery/klsf_long_dragon" + date,
		type: "GET",
		data: {
			"lotCode": lotCode
		},
	...
	if(data.errorCode == 0) {
		if(data.result.businessCode == 0) {
			...
			$(data).each(function() {
				...
				divhtml += '<div class="rightdiv padl0" style="margin-top:0.1rem;padding-left:0;">';
				divhtml += '<div class="haomali"><ul id="" class="cqnclilist">';
				...
				$(preDrawCode).each(function(index) {
					...
					/*if(index == preDrawCode.length - 1) {
						divhtml += "<li class='li_after ncnum" + this + "'><i style='font-size:10px;display:none;background:none'>" + this + "</i></li>";
					} else {
						divhtml += "<li class='ncnum" + this + "'><i style='font-size:10px;display:none;'>" + this + "</i></li>";
					}*/
					if(index == preDrawCode.length - 1) {
						divhtml += '<li><span class="ncnum' + this + '"><i style="display:none;">' + this + '</i></span></li>';
					} else {
						divhtml += '<li><span class="ncnum' + this + '"><i style="display:none;">' + this + '</i></span></li>';
					}
				});
				divhtml += '</ul></div>';
				divhtml += '<div class="longhuli displaynone"><ul  style="color:#666" class="ssclilistxt ssclilist listli lhlist listlitxt">';
...
method.selectedBS = function(obj, ifload) {
	...
	$("#numlist .haomali li").each(function(index) {
		...
		//是否为大小
		var ifdx = number >= 11 ? true : false;
		if(id == "xshm") {
			$(this).find("span").removeClass();
			$(this).find("span").addClass("ncnum" + $(this).find("i").text());
			//样式名为numsm+01到10
		} else if(id == "xsdx") {
			$(this).find("i").hide();
			$(this).find("span").removeClass();
			if(ifdx) {
				$(this).find("span").addClass("bluebig");
			} else {
				$(this).find("span").addClass("bluesm");
			}
		} else if(id == "xsds") {
			$(this).find("i").hide();
			$(this).find("span").removeClass();
			if(ifds) {
				$(this).find("span").addClass("blueeve");
			} else {
				$(this).find("span").addClass("bluesig");
			}
		}
	});
