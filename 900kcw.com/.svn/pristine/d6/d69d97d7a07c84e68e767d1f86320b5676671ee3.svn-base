...
function listData() {
	$.ajax({
		url: "http://www.900kcw.com/?r=home/count/gxklsf_count_double&date=",
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function lmmsMcMethod(obj) {
	...
	$("#lmsjtable>tbody tr").hide();
	$("#lmsjtable>tbody").find(".mc" + clicktxt).show();
}
//两面模式方法--筛选路珠
...
function forRank(data) {
	...
	var tdhtml = "";
	$("#lmsjtable>thead").empty(); //清空表头
	$("#lmsjtable>tbody").empty(); //清空表部
	var tablehead = "<tr height='50' class='mc1 first_th'><th  width='400'>日期</th><th width='300'>单</th><th width='300'>双</th><th width='300'>大</th><th width='300'>小</th></tr>";
	$("#lmsjtable>thead").append(tablehead); //添加表部
	//迭代list
	$(data).each(function(index) {
		var tddate = "<td width='400'>" + dateCut($(this)[0].date) + "&nbsp;&nbsp;&nbsp;&nbsp;" + $(this)[0].startIssue + "&nbsp;-&nbsp;" + $(this)[0].endIssue + "期</td>";
		var datlist = $(this)[0].list;
		$(datlist).each(function(index) {
			var tdd = "<td width='300'>" + $(this)[0].singleCount + "</td>";
			var tds = "<td width='300'>" + $(this)[0].doubleCount + "</td>";
			var tddc = "<td width='300'>" + $(this)[0].bigCount + "</td>";
			var tdx = "<td width='300'>" + $(this)[0].smallCount + "</td>";
			var tr = "<tr height='50' class='mc" + $(this)[0].rank + "'>" + tddate + "" + tdd + "" + tds + "" + tddc + "" + tdx + "</tr>";
			$("#lmsjtable>tbody").append(tr);
		})
	});
	//为tr追加斑马色
	for(var i = 0; i <= 10; i++) {
		$("#lmsjtable>tbody").find(".mc" + i + ":odd").css("background", "#fffaf5");
	}
}
