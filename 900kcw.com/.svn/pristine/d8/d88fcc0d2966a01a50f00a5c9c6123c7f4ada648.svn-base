...
function listData() {
	...
	$.ajax({
		url: "http://www.900kcw.com/?r=home/count/klsf_history_fornumber",
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	*/
	...
	$(data).each(function(index) {
		var trhtml = "";
		trhtml += "<td style='padding:0 10px'>" + this.date + "</td>"; //装载号码

		$(this.dataArrays).each(function(index) {
			trhtml += "<td>" + this + "</td>";
		});
		$("#dsdxlstable tbody").append("<tr>" + trhtml + "</tr>");
	});
}
