...
function listData(time, periods) {
	...
	$.ajax({
		url: "http://www.900kcw.com/?r=home/count/klsf_num_trend&periods=" + periods + "&date=" + time,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	$(data.title).each(function(index) {
		//不同的索引添加不同的表头
		$(this.appearCount).each(function(index) {
			if(index == 20){
				tr1 += "<td class='font14'>" + " " + "</td>";
			} else{
				tr1 += "<td class='font14'>" + Math.abs(this) + "</td>";
			}
		})
		$(this.averageMissingValues).each(function(index) {
			if(index == 20){
				tr2 += "<td class='font14'>" + " " + "</td>";
			} else{
				tr2 += "<td class='font14'>" + Math.abs(this) + "</td>";
			}
		})
		$(this.currentMissingValues).each(function(index) {
			if(index == 20){
				tr3 += "<td class='font14'>" + " " + "</td>";				
			} else{
				tr3 += "<td class='font14'>" + Math.abs(this) + "</td>";				
			}
		})
		$(this.maxAppearValues).each(function(index) {
			if(index == 20){
				tr4 += "<td class='font14'>" + " " + "</td>";				
			} else{
				tr4 += "<td class='font14'>" + Math.abs(this) + "</td>";				
			}
		})
		$(this.maxMissingValues).each(function(index) {
			if(index == 20){
				tr5 += "<td class='font14'>" + " " + "</td>";				
			} else{
			...
			$(this).find("td").each(
				function(index) {
					...
					if($(this).attr("title") == "0") {
						...
						} else if($(this).index() >= 23 && $(this).index() <= 25) {
