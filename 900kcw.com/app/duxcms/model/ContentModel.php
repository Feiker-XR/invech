...
class ContentModel extends BaseModel {
    ...
    public function stringToArray($string)
    {
     ...
     public function createQueryString($arr=NULL, $is_encode=false ) {
        $arg = '';
        foreach( $arr as $key => $value ) {
            if($is_encode) {
                $key = urlencode($key);
    ...
    public function route($field,$param,$id,$url_info,$select)
    {
        //初始化参与筛选字段
        $fields = array();
        ...
        $url_info['info'] = self::createQueryString($param); 
        return self::creat_url($url_info);
    }
     ...
     * @return bool 删除状态
     */
    public function creat_url($info)
    {
        return url(strtolower($info['app']).'/Category/filter',array('info'=>$info['info'],'class_id'=>$info['class_id'],'class_urlname'=>$info['urlname']));
    }
