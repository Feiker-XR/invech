...
syxwV.Data = function(data,n) {
	...
	for(var i = 0; i < numArr.length; i++) {
		if(i == 0) {
			tdliFirst = "<li class='codetwRed" + numArr[i] + "'></li>"
		} else {
			if(numArr[i] < 10) {
				numCode = "0" + numArr[i];
...
syxwV.stopFun = function(arrObg) {
	...
	var countLeft = 87;
	var flag = true;
	var checkRed = "twRed"; //随机匹配生成的红球类名
	...
	for(var i = 0; i < createArr.length; i++) {
		if(i == 0) { //生成红球
			var redTdLi = "<li class='twRed" + createArr[i] + "'></li>";
			$(".pipkai:nth-child(1)>ul").append(redTdLi);
...
syxwV.stopFunTry = function(arrObg){
	...
	for(var i = 0; i < createArrTry.length; i++) {
		if(i == 0) { //生成红球
			var redTdLi = "<li class='twRed" + createArrTry[i] + "'></li>";
			$(".pipkai:nth-child(1)>ul").append(redTdLi);
...
syxwV.redAnimate = function(objClass) {
	var k = $(objClass[0]).attr("class").toString().replace(/twRed/g,"")*1;
	if(k>8){
		k=0
	}else{
		k=1
	}
	$(objClass[k]).addClass("checkBall").animate({
		"top": "120px",
		"left": "7px"
	}, 500).animate({
		"top": "0px",
