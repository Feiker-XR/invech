...
function listData(date) {
	var action = date == undefined ? null : "date=" + date;
	$.ajax({
		url: urlbublic + "CQShiCai/queryComprehensiveRoadBead.php?" + action,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function repeatGetData(preIssue) {
	$.ajax({
		url: urlbublic + "pks/getRoadBeadStateByIssue.php?preIssue=" + preIssue,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(rank, data) {
	...
	$boxnumb.find(".ifgj").text(typeOf("qiu", data.rank));
