...
function listData() {
	...
	$.ajax({
		url: urlbublic + "CQShiCai/queryCQShiCaiTrendByLocation.php?num=" + num + "&issue=" + issue + "&date=" + time,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function checkNum() {
	...
	$("#table_weizhi").find(".fpredrawSpan span:nth-child(" + num + ")").siblings().removeClass("colorred");
	$("#table_weizhi").find(".fpredrawSpan span:nth-child(" + num + ")").addClass("colorred");
}
...
function forRank(data) {
	...
	$(data).each(function(index) {
		...
		$(this.bodyList).each(function() {
			var preIssue = '<td>' + this.issue + '</td>'; //期号
			var fpredrawSpan = "";
			//分解开奖号码
			var drawCode = this.code.split(",");
