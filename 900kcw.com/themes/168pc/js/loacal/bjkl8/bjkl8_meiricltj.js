...
/**
 * days 天数，默认为30天 
 * type 类型 1.单长龙;2.双长龙;3.大长龙;4.小长龙;5.和长龙
 * rank 名次,1总和
 **/
function listData() {
	...
	//得到TAB筛选条件类型 1.单长龙;2.双长龙;3.大长龙;4.小长龙;5.和长龙
	...
	var rank = "";
	//获取选择的排名或者龙虎
	if(typeshow == 0) { //当为大小单双时
		rank = $("#dxdsmc").find("li[class*='checked']").find("span").text();
		$("#totoli span").hide()
		$("#totoli").show();
	} 
	$.ajax({
		url: urlbublic + "LuckTwenty/queryLuckTwentyDailyDragon.php?type=" + type + "&rank=" + rank,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
