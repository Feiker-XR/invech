...
function listData(date) {
	...
	$.ajax({
		url: urlbublic + "LuckTwenty/queryComprehensiveRoadBead.php?" + action,
		type: "GET",
		data: {
			"lotCode": lotCode
		},
...
function repeatGetData(preIssue) {
	$.ajax({
		url: urlbublic + "LuckTwenty/getRoadBeadStateByIssue.php?preIssue=" + preIssue,
		type: "GET",
		data: {
			"lotCode": lotCode
		},
...
function forRank(rank, data) {
	...
	if(rank == 0) { //#box01
		...
		$boxnumb.find(".phpuble_count").text(data.totals[0]);
...
//		debugger
		...
		$boxnumb.find(".phpuble_more").text(data.totals[1]);
