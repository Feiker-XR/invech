$(function() {
	...
	$(".numberbtn span").live("click", function() {
		...
		tools.bigOrSmallXync($(this).attr("id"), 10);
	});
	...
	$("#startVideo").on("click", function() {
		if($("#klsf").find(".kajianhao ul").find("li").length == 8) {
			if($("iframe")[0].contentWindow.ifopen) {
				return;
			}
			document.addEventListener('touchstart', function() {
				$("iframe")[0].contentWindow.cqncVideo.soundstatus();
			}, false);
			$("#videobox").animate({
				"z-index": "19999"
			}, 200, function() {
				$(".content").animate({
					...
					var numarr = [];
					console.log("打开视频code数组：" + numarr);
					var kainumarr = []; //开奖中状态时，拿到历史表格中第一条 开奖号码
					var kaiPreDrawIssue;
					$("#klsf").find(".kajianhao li").each(function() {
						numarr.push(parseInt($(this).text()));
					});
					var preDrawCode = [].slice.call(numarr);
					var preDrawIssue = $(".preDrawIssue").text();
					var counttime = sedonds;
					$("iframe")[0].contentWindow.ifopen = true;
					//判断打开视频时是否处于开奖中状态
					$("iframe")[0].contentWindow.cqncVideo.statusFun(preDrawIssue, numarr, counttime, true);
					/*if($("#timebox").find(".opening").css("display") == "none"){//开奖完成
					} else{//开奖中
						$("#jrsmhmtjTab").find("tr").eq("1").find("td").eq("2").find("ul>li").each(function(){
							kainumarr.push(parseInt($(this).text()));
						});
						kaiPreDrawIssue = $("#jrsmhmtjTab").find("tr").eq("1").find("td").eq("1").text();
						$("iframe")[0].contentWindow.cqncVideo.statusFun(kaiPreDrawIssue,kainumarr,0,true);
					}*/
	...
	$("#videobox .closevideo").on("click", function() {
		...
		//		$("iframe")[0].contentWindow.cqncVideo.soundstatus();//声音初始化
...
function listData(code) {
	$.ajax({
		url: "http://www.900kcw.com/?r=home/history/xync&=" + code,
		type: "GET",
		// data: {
		// 	"lotCode": lotCode
		// },
...
function todayData() {
	$.ajax({
		url: "http://www.900kcw.com/?r=home/lottery/klsf_double_count",
		type: "GET",
		data: {
			"lotCode": 10009
		},
...
function longData() {
	$.ajax({
		url: "http://www.900kcw.com/?r=home/lottery/klsf_long_dragon&date=",
		type: "GET",
		data: {
			"lotCode": 10009
		},
...
function createHtmlList(jsondata) {
	...
	$("#jrsmhmtj>table").html('<tr><th width=150>时间</th><th width=80>期数</th><th id="numberbtn" class="numberbtn" width=386><span id="xshm" class="spanselect">显示号码</span><span id="xsdx">显示大小</span><span id="xsds">显示单双</span></th><th colspan="3">总和</th><th>尾大小</th><th colspan="4">龙虎</th></tr>');
	$(data).each(function(index) {
		...
		$(preDrawCode).each(function(index) {
			if(index >= (preDrawCode.length) - 1) {
				tdli += "<li class='ncnum" + this + "' style='color:#ffffff;border:none;margin-right:0;margin-left:0;margin-top:5px'><i style='font-size:10px;display:none;'>" + this + "</i></li>";
			} else {
				tdli += "<li class='ncnum" + this + "' style='color:#ffffff;border:none;margin-right:13px;margin-left:0;margin-top:5px'><i style='font-size:10px;display:none;'>" + this + "</i></li>";
			}
		});
		var stylestr = "style='color:";
		var tdnumber = "<td class='blueqiu'><ul>" + tdli + "</ul></td>";
		if(config.ifdebug) {
			console.log("开奖号码：" + preDrawCode.length);
		}
