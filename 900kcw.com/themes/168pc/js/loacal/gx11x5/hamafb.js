...
function listData(time, periods) {
	...
	$.ajax({
		url: urlbublic + "ElevenFive/queryElevnFiveNumberTrend.php?issue=" + periods + "&date=" + time,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	$(data[0].bodyList).each(function(index) {
		...
		var tdspanr = "</td>";
		var preDrawCode = "";
		$(this.code.split(",")).each(function() {
			preDrawCode += '<span class="kaijnum" style="margin-right:8px">' + this + '</span>';
		})
		...
		var rank1 = "",
			...
			rank4 = "",
			rank5 = "";
		var preDrawCodes = $(this.code.split(","));
		var	thisVaule = "";
		$(this.array).each(function(index) {
			...
			if(index <= 10){
				var Index = index+1;
				$(preDrawCodes).each(function(index){
					thisVaule = preDrawCodes[index].substr(0,1)==0 ? preDrawCodes[index].substr(1,2) : preDrawCodes[index];
					if(thisVaule==Index){
						flag = true;
					}  
				});
				if(flag){
					title = "title='0'";
					rank1 += "<td " + title + "><span>" + (index*1+1) + "</span>" + tdspanr;
				}else{
					title = "class='rank1 yilou'";
					rank1 += "<td " + title + "><span>" + Math.abs(this) + "</span>" + tdspanr;
				}
			} else{
				if(this * 1 > 0) {
					title = "title='0'";
			...
			$(this).find("td").each(
				function(index) {
					...
					if($(this).attr("title") == "0") {
						if($(this).index() >= 2 && $(this).index() <= 12) {
						...
						} else if($(this).index() >= 14 && $(this).index() <= 25) {
							...
							if($(this).index() == 14) {
								...
								$(this).find("span").attr("title", $(this).find("span").text());
								$(this).find("span").text("尾大");
								$(this).find("span").css("background", color[10]);
							} else if($(this).index() == 21) {
								$(this).find("span").attr("title", $(this).find("span").text());
								$(this).find("span").text("尾小");
								$(this).find("span").css("background", color[10]);
							}
						}
					}
