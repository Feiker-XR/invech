...
function listData(time, periods) {
	...
	$.ajax({
		url: urlbublic + "ElevenFive/queryElevnFiveSumTrend.php?issue=" + periods + "&date=" + time,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	$(data[0].bodyList).each(function(index) {
		...
		var preDrawCode = "";
		$(code).each(function() {
			preDrawCode += '<span class="kaijnum" style="margin-left:5px">' + this + '&nbsp;</span>';
		})
