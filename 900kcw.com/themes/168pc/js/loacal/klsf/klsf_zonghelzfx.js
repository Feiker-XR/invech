...
function listData(date) {
	...
	$.ajax({
		// url: urlbublic + "klsf/queryComprehensiveRoadBead.php?date=" + date,
		url: "http://www.900kcw.com/?r=home/count/klsf_road_bead&date=" + date,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(rank, data) {
	...
	rank名次,
	1-8分别为第一至第八名,9为总和
	state形态: 如1.单双;2.大小;3.龙虎;4.尾大尾小;5.合单合双;6.中发白;7.东南西北
	totals数组第一个元素表示单、大、龙、尾大、合单、中、东;第二个元素表示双、小、虎、尾小、合双、发、南、;第三个元素表示白、西;第四个元素表示北roadBeads数组,
	1表示单、大、龙、尾大、合单;
	0表示双、小、虎、尾小、合双;
	2.表示中;
	3表示发,
	...
	if(data.state == "6") {
		...
		if(data.totals.length >= 3){
			$boxnumb.find(".left_count").text(data.totals[0]);
			$boxnumb.find(".right_count").text(data.totals[1]);
			$boxnumb.find(".middle_count").text(data.totals[2]);
		} else {
			$boxnumb.find(".left_count").text(data.totals[0]);
			$boxnumb.find(".right_count").text(data.totals[1]);
		}
	}
	$boxnumb.find(".ifgj").text(typeOf("qiu1", data.rank));
	...
	$(data.roadBeads).each(function(index) {
		...
		if(state == "1") {
			...
			newroadbeads = typeOf("dxhList",data.roadBeads[index]);		
		} else if(state == "3") {
		...
		if(newroadbeads == "双" || newroadbeads == "龙" || newroadbeads == "大" || newroadbeads == "双" || newroadbeads == "中" || newroadbeads == "南") {
...
function forRankYibu(rank, data) {
	...
	if(state == "1") {
		...
		newroadbeads = typeOf("dxh",data.value);
	} else if(state == "3") {
	...
	var $middle_countt = $boxnumb.find(".middle_countt");
	var leftxt = $left_count.text() * 1 + 1;
	var rigtxt = $right_count.text() * 1 + 1;
	var midtxt = $middle_count.text() * 1 + 1;
