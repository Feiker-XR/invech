...
////初始加载动画
var lotCode = lotCode.tjklsf;
//初始化界面定时器得到请求采种类别
var indexObj = new Object(); //创建对象 
function ajaxRequst(issue,id,flag) {
	indexObj.ajaxSsc(issue,id,flag);
}

indexObj.ajaxSsc = function(issue, id, flag) {
	issue = issue == undefined ? "" : issue;
	var iferror = false;
	var code = $('#klsf').find('.nextIssue').text();
	
	$.ajax({
		// url: urlbublic + "klsf/getLotteryInfo.php?issue=" + issue,
		url: "http://www.900kcw.com/?r=home/lottery/tjkl10f&num=" + code,
		type: "GET",
		// data: {
		// 	"lotCode":lotCode
		// },
...
function creatDataHead(jsondata, id) {
	publicmethod.insertHeadKlsf(jsondata,id);
}

function creatDataHead2(jsondata, id) {
	publicmethod.insertHeadKlsf_2(jsondata,id);
}
