...
function listData() {
	...
	$.ajax({
		url: urlbublic + "lotteryJSFastThree/getJSFastThreeNumberList.php?",
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	*/
	...
	$(data).each(function(index) {
		...
		var trhtml = "";
		trhtml += "<td>" + this.preDrawDate + "</td>"; //装载号码
		//		var listrank = this.list;
		//		$(listrank).each(function(index) {
		//			trhtml += "<td>" + this.counts + "</td>";
		//		});
		$("#dsdxlstable tbody").append("<tr>" + trhtml + "<td>" + this.oneTimes + "</td><td>" + this.twoTimes + "</td><td>" + this.threeTimes + "</td><td>" + this.fourTimes + "</td><td>" + this.fiveTimes + "</td><td>" + this.sixTimes + "</td><td>" + this.bigTimes + "</td><td>" + this.smallTimes + "</td></tr>");
	});
