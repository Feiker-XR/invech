...
function listData() {
	...
	$.ajax({
		url: urlbublic + "lotteryJSFastThree/queryOddAndEvenTrend.php?date=" + time + "&issue=" + issue + "&periods=" + periods,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	$(data).each(function(index) {
		...
		$(this.bodyList).each(function() {
			...
			$(this.oddEvenCompare).each(function(index) {
				var title = "";
				var bgqiu = "";
				var value = "";
				if(index == 0) {
			...
			$(this.oddNumber).each(function(index) {
				var title = "";
				var bgqiu = "";
				var value = "";
				if(this * 1 >= 0) {
			...
			$(this.evenNumber).each(function(index) {
				var title = "";
				var bgqiu = "";
				var value = "";
				if(this * 1 >= 0) {
		...
		//循环最大连出
		$(this.title).each(function(index) {
			//不同的索引添加不同的表头
			//循环第一行
