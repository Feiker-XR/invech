...
function listData(time) {
	...
	$.ajax({
		url: urlbublic + "lotteryJSFastThree/getRoadOfBeadTotal.php?date=" + time,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(rank, data) {
	...
	$boxnumb.find(".ifgj").text(typeOf("qiuonebig", data.rank));
	...
	$(data.roadBeads).each(function(index) {
		...
		var newroadbeads = "";
		if(state == "1") {
			//			newroadbeads = (data.roadBeads[index] == 0) ? "单" : "双";
			if(data.roadBeads[index] == 0) {
				newroadbeads = "单";
			} else if(data.roadBeads[index] == 1) {
				newroadbeads = "双";
			} else {
				newroadbeads = "通吃";
			}
		} else if(state == "2") {
			//			newroadbeads = (data.roadBeads[index] == 0) ? "大" : "小";
			if(data.roadBeads[index] == 0) {
				newroadbeads = "大";
			} else if(data.roadBeads[index] == 1) {
				newroadbeads = "小";
		...
		if(newroadbeads == "双" || newroadbeads == "通吃" || newroadbeads == "大") {
