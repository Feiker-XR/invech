...
function listData() {
	$.ajax({
		url: "http://www.900kcw.com/?r=home/count/pk10_history_forgyh",
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	/*businessCode
		String:业务状态码。100001:输入参数为空；100002:查询结果集为空，详情见错误列表
		date:日期
		gyhBig:冠亚和大的数量
		gyhDouble:冠亚和双的数量
		gyhSingle:冠亚和单的数量
	...
	*/
	$("#gyhlmlstable").empty();
	var thead = '<tr height="48" class="thead"><th>日期</th><th>冠亚和大</th><th>冠亚和小</th><th>冠亚和单</th><th>冠亚和双</th></tr>';
	$("#gyhlmlstable").append(thead);
	//迭代list
	$(data).each(function(index) {
		var tddate = "<td width='300' class='smthead'>" + this.date + "</td>";
		var gyhBig = "<td width='200'>" + this.gyhBig + "</td>";
		var gyhSmall = "<td width='200'>" + this.gyhSmall + "</td>";
		var gyhSingle = "<td width='200'>" + this.gyhSingle + "</td>";
		var gyhDouble = "<td width='200'>" + this.gyhDouble + "</td>";
		var tr = "<tr height='40'>" + tddate + "" + gyhBig + "" + gyhSmall + "" + gyhSingle + "" + gyhDouble + "</tr>"
		$("#gyhlmlstable").append(tr);
		$("#gyhlmlstable tbody tr:even td").css("background", "#fff9f2");
	});
}
