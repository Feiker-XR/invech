...
function listData(time) {
	...
	$.ajax({
		url: "http://www.900kcw.com/?r=home/count/pk10_road_bead_fb&date=" + time,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function repeatGetData(preIssue) {
	$.ajax({
		url: urlbublic + "pks/getFbRoadBeadStateByIssue.php?preIssue=" + preIssue,
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(rank, data) {
	...
	$boxnumb.find(".ifgj").text(typeOf(data.code, "rank"));
