...
function listData(time) {
	...
	$.ajax({
		url: urlbublic + "CQShiCai/queryDrawCodeHeatState.php?recentPeriods=20",
		type: "GET",
		data: {
			"lotCode":lotCode
		},
...
function forRank(data) {
	...
	$(data).each(function(index) {
		...
		$(listr).each(function() {
			lir += "<li class='sscnumblue'>" + $(this)[0].drawCode + "<i>" + $(this)[0].count + "</i></li>";
		});
		$(listw).each(function() {
			liw += "<li class='sscnumblue'>" + $(this)[0].drawCode + "</li>";
		});
		$(listl).each(function() {
			lll += "<li class='sscnumblue'>" + $(this)[0].drawCode + "</li>";
		});
