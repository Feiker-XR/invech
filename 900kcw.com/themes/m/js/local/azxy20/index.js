$(function() {
	...
	$("#startVideo").on("touchstart", function() {
		$("iframe")[0].contentWindow.syxwV.startVid(tools.bjkl8DataStr());
		startVideo();
		$("iframe")[0].contentWindow.ifopen = true;
	});
	document.addEventListener('click', function() {
		if($("iframe")[0].contentWindow.ifopen) {
			$("iframe")[0].contentWindow.syxwV.sound.play("audioidKai");
			$("iframe")[0].contentWindow.syxwV.sound.play("audioidBg");
...
method.listData = function(date) {
	$.ajax({
		url: "http://m.900kcw.com/?r=home/history/azxy20",
		type: "GET",
		// data: {
		// 	"lotCode": lotCode
		// },
...
/*method.todayData = function(date) {
		$.ajax({
			url: config.publicUrl + "CQShiCai/queryDoubleNumber.php?date=" + date,
			type: "GET",
			data: {
				"lotCode": lotCode
			},
...
/*method.longData = function(date) {
	$.ajax({
		url: config.publicUrl + "CQShiCai/getShiCaiDailyDragonCount.php?date=" + date,
		type: "GET",
		data: {
			"lotCode": lotCode
		},
...
method.createHtmlList = function(jsondata) {
	...
	if(data.errorCode == 0) {
		if(data.result.businessCode == 0) {
			...
			$(data).each(function() {
				var divhtml = "";
				divhtml += '<div class="listotherline bjkl8_listotherline bortop001 ssclist">';
				...
				divhtml += '<div class="haomali"><ul id="hoamaul" class="ssclilist listli zhxtlilist">';
				...
				$(preDrawCode).each(function(index) {
					...
					if(index == preDrawCode.length - 1) {
						divhtml += '<li><span class="Orange"><i>' + this + '</i></span></li>';
					} else {
						if(this > 40) {
							divhtml += '<li><span class="bluenum"><i>' + this + '</i></span></li>';
						} else {
