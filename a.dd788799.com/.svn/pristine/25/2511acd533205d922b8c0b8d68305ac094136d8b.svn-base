...
function codeplay(code_arr){
	var _html='';
	if(game.type==20|| game.type==61|| game.type==73|| game.type==74|| game.type==71|| game.type==72||game.type==73|| game.type==65||code_arr.length==66){//正在开奖中彩球大小
...
function gamePostedCode(err, data){
	if(err){
		$.confirm('您的可用资金不足，是否充值？',function (data){
