...
if (! function_exists('hashid')) {
    ...
    {
        $class = bong\service\HashidsManager::class;
...
if (! function_exists('dehashid')) {
    function dehashid($id,$hasher='default')
    {
        ...
        return is_array($data) && count($data) == 1 ? $data[0] : false;        
    }
}
...
function transaction($callback,$throw=true){
...
function get_where_params($ignore = []){
