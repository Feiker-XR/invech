$(function () {
    ...
    $('#lt_trace_ok').live('click', function () {
        var no_count = $('#lt_trace_count_input').val(); //期数
        var beishu = $('#lt_trace_times_margin').val(); //倍数
        var money = parseFloat($('#lt_cf_money').text()); //投注金额
...
//快捷金额input  moved by feiker
    $("#toptouzhu .kjanniu a").live('click', function () {
        ...
        // 快钱或者是香港六合彩
        !!$('#inputMoney').length ? $('#inputMoney').val(num) : $('#kj_input').val(num);
    });

    $('#tab_t li').live('click', function () {
        $(this).addClass('act');
        $(this).siblings().removeClass('act');
        var index = $(this).index();
        $('#tab_c').children().eq(index).show();
        $('#tab_c').children().eq(index).siblings().hide();
    });
...
function gamePostCode() {
    ...
    var gameType = 0; // 默认是官方玩法, 1表示快钱玩法


    if ($('#tabbar-div-s4 span').hasClass('tab-front')) {		// 说明是快钱玩法
        gameType = 1;
    }
    console.log('gameType', gameType)
    // 说明是官方玩法
    if (gameType == 0 && parseFloat($('#lt_sel_nums').text()) < 1) {
    ...
    try {
        var result = gameActionAddCode(1);
        $.isArray(result) ? code = result : code[0] = result;
    } catch (err) {
        return false;
    }

    if (typeof code == 'undefined' || typeof code[0] == 'undefined') {
        return false;
    }
    ...
    actionNo = actionNo.replace('-', '');	//过滤掉开奖期号中的"-" 符号 例如 20170611-20  2017061120
    ...
    wait();
    // 打印提交的数据
    debug('gamePostCode方法-postAll接口-提交之前的数据:')
    debug({
        code: code,
        para: data,
        zhuiHao: zhuiHao
    ...
    var tipString = '<h4><b><span class="ui-wjicon-confirm"></span>您确定要购买第' + actionNo + '期彩票？</b></h4>';
    tipString += '<br /><div class="tz-table"><table width="100%"><tr><th>玩法</th><th>号码</th><th>注数</th><th>倍数</th><th>模式</th></tr>';
...
function gamePostCode2() {
    ...
    var tipString = '<div class="tz-title"><span><span class="ui-wjicon-confirm"></span>您确定要购买第 ' + actionNo + ' 期彩票？</span></div>';
    tipString += '<div class="tz-table"><table width="100%"><tr><td>玩法</td><td>号码</td><td>注数</td><td>倍数</td><td>模式</td></tr>';
    ...
    tipString += '</table></div>';
    tipString += '<br /><span style="color: #f87b1f;">' + '共 ' + $('#lt_cf_nums').text() + ' 注，总金额：' + $('#lt_cf_money').text() + ' 元 </span>';
    ...
    $('#wanjinDialog').html(tipString).dialog({
        ...
        buttons: {
            "确定购买": function () {
                ...
                data['type'] = game.type;
                data['actionNo'] = actionNo;
                data2 = {code: code, para: data, zhuiHao: zhuiHao};
                wait();
                debug('gamePostCode2方法-postAll接口-提交前的数据:');
                debug(data2);
                $.ajax('/index.php/game/postAll', {
                    data: data2,
                    type: 'post',
...
function gamePostCode3() {
    ...
    $('#lt_trace_issues_table tr').each(function () {
        ...
        if (child.checked) {
            ...
            //  这里加上了对空的判断,如果值为空则不处理 actionNo += node.innerHTML + '|';
            if (node.innerHTML != '') {
    ...
    console.log(beishu);
    actionNo = actionNo.replace(/(^\s*)|(\s*$)/g, ""); //过滤空格
    ...
    $('#wanjinDialog').html(tipString).dialog({
        ...
        buttons: {
            "确定购买": function () {
                ...
                wait();
                debug('gamePostCode3方法-postAll接口-提交前的数据:');
                debug({
                    code: code,
                    para: data,
                    zhuiHao: zhuiHao
...
function reloadMemberInfo() {
    ...
    //parent.window.autoupdate('/index/userinfo');
    //改为pusher
    // $.ajax({
    // type: "POST",
    ...
    // //document.location.href = "/common/logout";
    // return;
    // }
    // },
    // error: null,
...
function gameFreshOrdered(err, msg) {
    if (err) {
        ...
        $('#order-history').load('/index.php/game/getOrdered');
    }
}
...
function freshKaiJiangData(type) {
    $('#historylot').load('/index.php/index/getHistoryData/type/' + type);
}
...
function gameActionAddCode(type) {
    ...
    var $slider = $('#lt_sel_dyprize');		// 奖金 返点

    var obj, $game = $('#lt_selector .pp'),  // $game --> table
        calcFun = $game.attr('action');		// table下对应的action函数

    if (calcFun && (calcFun = window[calcFun]) && (typeof calcFun == 'function')) {
        try {
            obj = calcFun.call($game);
            if ($.isArray(obj) && obj.length == 0) {			// 如果选择为空
                winjinAlert('请至少选择一组注单', 'alert');
            }
            // 单笔投注注数限额
...
function gameAddCode(code, type) {
    if (type == 3) {
        return gameAddKqwfCode(code, type);
    }
    if ($.isArray(code)) {
        var arr = [];
        for (var i = 0; i < code.length; i++) {
            arr.push(gameAddCode(code[i], 1));
        }
        return arr;
    }
    ...
    var jiangjin = document.getElementById("lt_sel_dyprize").value; //获取奖金

    try {
        code = $.extend({
            // 反点
            fanDian: parseFloat(jiangjin.split('-')[1].replace("%", "")),
        ...
        if ($wei.length) {
            if ($(':checked', $wei).length != weiCount) throw('请选择' + weiCount + '个位置！');
        ...
        if (type == 2) {
            ...
            $('<tr>').data('code', code)
                ...
                .append(
                    // 号码列表
                    $('<td class="code-list" title=" ' + code.actionData + '">').append(wei + (code.actionData.length > 18 ? (code.actionData.substr(0, 5) + '...') : code.actionData))
                )
                .append(
                    // 注数
                ...
                .append(
                    ...
                    $('<td>').append(code.mode.toFixed(2) + "元")
                )
                .append(
                    // 总金额
...
function gameAddKqwfCode(code, type) {
    if ($.isArray(code)) {
        for (var i = 0; i < code.length; i++) gameAddKqwfCode(code[i], type);
        return;
    }
    ...
    //plid,bonusProp,mode 有统计函数(action指定)返回
    try {
        code = $.extend({
            // 反点
            fanDian: 0.0,
        ...
        playedName = code.playedName || $('#tabbar-div-s4 .tab-front .content').text(),
            code.playedName = playedName;
        //玩法名在隐藏域中
        wei = '';
        ...
        if (type == 3) {
            ...
            $('<tr>').data('code', code)
                ...
                .append(
                    // 单价
                    //$('<td>').append(modeName[code.mode])
                    $('<td>').append(parseFloat(code.mode).toFixed(2) + "元")
                )
                .append(
                    // 总金额
                    $('<td>').data('value', amount).append(amount.toFixed(2) + "元")
...
function wait() {
    var html = '<img src="' + '/static/theme/sscTheme/theme1/img/wait.gif" />';
...
function gameSetPl(value, flag, fanDianBdw) {
	...
	if(fanDianBdw){
		...
		var myfandian=parseFloat($dom.attr('fan-dian'));
		var fandians = [] , bases = [];
		$.each(value,function(key,pair){
			var prop = parseFloat(pair.bonusProp);
			var base = parseFloat(pair.bonusPropBase);
			var fandian = ((prop-base)/maxfd*myfandian + base).toFixed(2);
			fandians.push(fandian);
			bases.push(base.toFixed(2));
		});
		var fan1 = fandians.join('|') + "-" + "0.0%";
		var fan2 = bases.join('|') + "-" + myfandian.toFixed(1) + "%";
	...
	*/
    ...
    $('#lt_sel_dyprize').attr('fan-dian', fandianBili);
    $.each(gfwf_pls_pair, function (key, pair) {
        var prop = parseFloat(pair.bonusProp);
        var base = parseFloat(pair.bonusPropBase);
        var convertBlMoney = (prop - base) / maxFandian;
        //convertBlMoneys.push(fandian);

        // 赔率 = 最大配率 - 返点比例 * 转换比例
        var pl = (prop - fandianBili * convertBlMoney).toFixed(3);
        pls.push(pl);
    });
    var plStr = pls.join('|');
    //$("#jiangjin-change").data("value",plStr);

    // 渲染界面中赔率部分
    $("#jiangjin-change").html(plStr);
    $("#jiangjin-change").data("plStr", plStr);

    $('#lt_sel_dyprize').attr('maxfd', plStr);
    $('#lt_sel_dyprize option').val(plStr + '-' + fandianBili + '%');
}
...
function gameKanJiangDataC(diffTime) {
    ...
    if (diffTime < 0) {
        // 时间小于0时,弹窗提示是否清空已选择内容
        var tipString = '<b><span class="ui-wjicon-confirm"></span>清空预选内容？</b>';
        $('#wanjinDialog').html(tipString).dialog({
            title: '投注提示',
            resizable: false,
            width: 500,
        ...
        var cancelTimer = setInterval(function () {
            if (countTime < 0) {
                ...
                $('.ui-dialog-buttonset').find('button').eq(1).text('取消').click();
            } else {
                $('.ui-dialog-buttonset').find('button').eq(1).text('取消(' + countTime + ')');
                countTime--;
            }
        }, 1000)
        ...
        // 时
        if (h < 10) {
            $('#leftTime .s .sp1').text(0)
            $('#leftTime .s .sp2').text(h)
        } else {
            $('#leftTime .s .sp2').text(Math.floor(h / 10))
            $('#leftTime .s .sp2').text((h % 10))
        }
        // 分
        if (s < 10) {
            $('#leftTime .f .sp1').text(0)
            $('#leftTime .f .sp2').text(s)
        } else {
            $('#leftTime .f  .sp1').text(Math.floor(s / 10))
            $('#leftTime .f  .sp2').text((s % 10))
        }
        // 秒
        if(m < 10) {
            $('#leftTime .m  .sp1').text(0)
            $('#leftTime .m  .sp2').text(m)
        } else {
            $('#leftTime .m  .sp1').text(Math.floor(m / 10))
            $('#leftTime .m  .sp2').text((m % 10))
        }

        if (T) clearTimeout(T);
        T = setTimeout(gameKanJiangDataC, 1000, 0);
    }
}

function loadKjData() {
    var type = game.type;
    /*$.ajax('/index.php/index/getLastKjData/type/'+type,{
		dataType:'json',
		cache:false,
		error:function(){
		...
		success:function(data){
			...
			if(!data){
				...
				try{
					...
					$('#last_issue').html(data.actionNo); //更新期号
    ...
    $.ajax('/index/getLotteryQiHao?type=' + type, {
        ...
        success: function (data) {
            //console.log('获取开奖号码');
            //console.log(data);
            $('#last_issue').html(data.phase); //更新期号
            if (!data.data) {
                if (kjTimer) clearTimeout(kjTimer);
                kjTimer = setTimeout(loadKjData, 10000);
                setKjing();
...
function getQiHao() {
    ...
    /*$.getJSON('/index.php/index/getQiHao/type/'+game.type, function(data){
	    console.log('获取期号');
		console.log(data);
		if(data && data.lastNo && data.thisNo){
			...
			var _diff = data.diffTime-kjTime < 0 ? data.diffTime-kjTime  : 1;
			gameKanJiangDataC(_diff, data.thisNo.actionNo);
		}
	},function () {
        console.log('获取期号 -- 失败');
    });*/
    $.getJSON('/index/getBetQiHao?type=' + game.type, function (data) {
        console.log(data);
        $('#current_issue').html(data.phase);
        $('#number').html(data.phase);
        $('#tip').html(data.phase);
        //futureNo(data.phase,game.type);
        // 未来期
        // 未来期
        futureNo(data.future_phases);
        if (T) {
            clearTimeout(T);
        }
        gameKanJiangDataC(data.fptime, data.phase);
    });
}
...
function futureNoOld(action, type) {
    var html = '';
    ...
    // 此种期号 20180201-17 或者 20180201-017
    if (classtype == 1) {
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (num <= maxNo) {
                no2 = qian + '-' + (no.length == 2 ? (num + 1000 + ' ').substr(2, 3) : (num + 1000 + ' ').substr(1, 3));
                ...
                no2 = qian + '-' + (no.length == 2 ? (num + ' ').substr(2, 3) : (num + ' ').substr(1, 3));
                html = html + '<option value="' + no2 + '">' + no2 + '</option>';
            }
...
function selectGroup(self) {
    ...
    $('#played-span').load(url, function () {
        ...
        // 找到当前选中的 span   ???    选中老的?
        var $old = $(self).parent('.u_tab_div').find('.tab-front');
        //
        // // 给当前加上选中
        $(self).removeClass('tab-back').addClass('tab-front');
        //
        // // 兄弟节点去掉选中
        $(self).parent().siblings('.u_tab_div').find('.tab-front').removeClass('tab-front').addClass('tab-back');
        // // 给玩法组 加上标记  on      官方玩法  块钱玩法
        $(self).siblings('.tab_list_title').addClass('on').parent('.u_tab_div').siblings('.u_tab_div').find('.tab_list_title').removeClass('on');
        // // 以前选中的去掉选中状态
        $old.removeClass('tab-front').addClass('tab-back');
        // // 找到快钱玩法
        $kqwf = $(self).parents(".u_tab_div.kqwf");

        //判断  是是否选中的块钱玩法
        if ($kqwf.length) {
            // 去掉 选好了 的 click 事件  添加方法
            $('#lt_sel_insert').unbind('click').attr('onclick', '').click(function () {
                // 预投注
                gameActionAddCode(3);
            });
            // 隐藏选注
...
function selectPlayed(self) {
    ...
    $('#played-content').load(url, function (re) {
        destroyWait();
        var $old = $(self).parent('.tz_li').siblings('.tz_li').find('.act');
        $(self).removeClass('back').addClass('act');
        $(self).siblings('div').removeClass('act').addClass('back');
        $old.removeClass('act').addClass('back');
        var a = re;
    });
...
// 设置快捷选择 by feiker
function setQuickPick(self) {
    var _inputMoney = $('#inputMoney').val();	// 快捷金额
    var _self = $(self).closest('td');			// td自身
    var _childs = [_self];
    var _last;

    if (_self.next().find('input').length == 0) { // 说明下一个不是输入项
...
// 快钱玩法快速选择器
function kqQuick() {
    $('.kq_sm_1,.kq_sm_2').click(function () {
        ...
        if (isChoosed == 'choosed') {
            ...
            smThis.css({'background': 'white'});
            smThis.siblings('li').not('.kq_sm_3').css({'background': 'white'});
            smThis.siblings('.kq_sm_3').find('input').val('');
        } else {
            smThis.attr('ischoosen', 'choosed');
            smThis.css({'background': 'yellow'});
            smThis.siblings('li').not('.kq_sm_3').css({'background': 'yellow'});
