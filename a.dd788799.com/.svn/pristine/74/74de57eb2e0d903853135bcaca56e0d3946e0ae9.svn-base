...
class Query
{
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
    	...
    	if($list_rows = input('list_rows/i')??0){
    		$listRows = $list_rows;
    	}
     ...
     * @return boolean
     */
    public function chunk_bak($count, $callback, $column = null)
    {
        $options = $this->getOptions();
    ...
    private function empty($resultSet){
        ...
        throw new \Exception('结果集不是数组或者集合!');
    }

    private function end($resultSet){
        if(is_array($resultSet)){
    ...
    public function chunk($count, $callback, $column = null)
    {
        ...
        while (!$this->empty($resultSet)) {
            if (false === call_user_func($callback, $resultSet)) {
                return false;
            }
            $end       = $this->end($resultSet);
            $lastId    = is_array($end) ? $end[$key] : $end->$key;
            $resultSet = $this->options($options)
            ->limit($count)
            ->bind($bind)
