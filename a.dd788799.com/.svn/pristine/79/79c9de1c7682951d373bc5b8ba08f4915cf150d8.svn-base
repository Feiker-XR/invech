...
function  sscnxzh($betData, $kjData) {
    ...
    if(count(_arr($betData)) != count(_arr($kjData))){
        return [];
    }
    ...
    $remain_count = count(_arr($kjData)) - count($cnts);
    for($i=0;$i<$remain_count;$i++){
        array_unshift($cnts,0);
    }
 ...
 * @return [type]          [description]
 */
function pk10_Q3f ($betData,$kjData) {
    //$kjData = removeFromList($kjData,',',4,5) ;
    $kjData = _qh($kjData,3) ;
    return pk10_fs($betData,$kjData) ;
}
 ...
 * @return [type]          [description]
 */
function sxzxR3h_old($betData, $kjData, $w){
    $kjData = rx_wym($w, $kjData);
    return z36($betData, $kjData);
}
...
function sxzxQ3_hz($betData, $kjData){
    ...
    if(in_array($hz, _arr_no_split($betData))){
        if($z6){
            return [0,1];
        }else{
            return [1,0];
...
function sxwfQ3_hzws($betData, $kjData){
    ...
    return (int)in_array($hzws, _arr_no_split($betData));
}
 ...
 * @return [type]          [description]
 */
function rxzxR2d2($betData, $kjData, $w) {
/*
...
function rxzxR2d($betData, $kjData) {    
    return z2d($betData, $kjData);
}
...
function pk10_zxQ3f($betData, $kjData) {
   // $betData = trim_zero($betData);
    return pk10_Q3f($betData, $kjData);
}
 ...
 * @return          返回中奖注数
 */
function pk10_fs($bet,$kjData,$fn='compare_digit',$reject = null) {
    $bets = array();
...
function z3($betData,$kjData) {
    ...
    if (strpos($betData,'|')!==false) {
        ...
        //return (int)array_contain(_arr($betData),_arr($kjData));
        return (int)array_include(_arr($betData),_arr($kjData));
    }
}
...
//sxzxR3h '1,-,2,-,3|4,-,5,-,4'
function z36_old($betData,$kjData) {
...
function z36($betData,$kjData) {
    ...
    foreach($bets as $bet){
        $kjTmp = $kjData;
        rx_tpf_trim($bet,$kjTmp);

        $z6 = count(array_unique(_arr($kjTmp)))==3;
        
        $z6_bet = count(array_unique(_arr($bet)))==3;
        
        if($z6&&$z6_bet){
            if(array_equal($bet,$kjTmp)){
                $z6_cnt ++ ;
            }
        }
