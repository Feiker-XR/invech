...
//    );
   ...
   /* 自动验证规则 */
   protected $_validate = array(
        ['name'   => 'require|/^[a-zA-Z]\w{0,39}$/','行为标识必须|标识不合法'],
        ['title'  => 'require|length:1,80','标题不能为空|标题长度不能超过80个字符'],
        ['remark' => 'require|length:1,140','行为描述不能为空|行为描述不能超过140个字符']
   );
   
   /* 自动完成规则 */
    protected $_auto = array(
        ['status'=>1],
        ['update_time'=>'dateFormat:y-m-d|数据格式有误']
    );
