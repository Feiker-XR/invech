<?php
namespace bong\service\captcha;

use think\Session;
/*
use JsonSerializable;

class DataUrl implements JsonSerializable{
    public function __construct($content)
    {
        $this->content = self::toDataUrl($content);
    }

    public static function toDataUrl($content){
        $mime = 'image/png';
...
class Captcha
{
    ...
    public function check_api($code, $key)
    {
        $cache_key = $this->_prefix.$key;
        $cache_code = cache($cache_key)??null;
        if(!$value){
            return false;
        }

        //if(bcrypt_verify($code,$key)){//碰撞的问题
        if($code == $cache_code){
    ...
    public function entry_api()
    {
        ...
        //$secode['verify_time'] = time(); // 验证码创建时间
        cache($cache_key,$code,$this->expire);
        ...
        return ['key'=>$key, 'img'=>$this->toDataUrl($content),];
        //return ['key'=>$key, 'img'=> (new DataUrl($content)),];
        //$content = json_encode(['a'=>'a','b'=>'b',]);
    ...
    public function toDataUrl($content){
        $mime = 'image/png';
