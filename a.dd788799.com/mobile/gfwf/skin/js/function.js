...
function setGameZhuiHao(data) {
    ...
    wait();
    var actionNo = $.parseJSON($.ajax('/index/getQiHao/type/' + data.type, {async: false}).responseText);
    destroyWait();
    $.get('/game/zhuiHaoModal', function (html) {
        // console.log(html)
        $(html).dialog({
            title: '<div class="ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix">追号期数：<span class="qs">0</span>　总金额：<span class="amount">0.00</span>元</div>',
            height: 300,
            modal: true,
            stack: false,
            ...
            open: function (event, ui) {
                ...
                str += 'type/' + data.type + '/';
                str += 'price/' + price + '/';
                str += 'beishu/' + data.beiShu + '/';
                str += 'actionNum/' + actionNo.thisNo.actionNo + '/';
                $this.attr('src', '/game/zhuiHaoQs/' + str);
                $('.tr-cont', this).load($this.attr('src') + 'count/' + 10);
                $this.closest('.zhui-hao-modal').find('select:first').change(function () {
                    $('tbody', $this).load($this.attr('src') + 'count/' + this.value);
                });
            }
        });
    });
...
function gameMsgAutoTip() {
    ...
    if (calcFun && (calcFun = window[calcFun]) && (typeof calcFun == 'function')) {
        try {
            ...
            var amount = gameGetMode() * gameGetBeiShu() * obj.actionNum
...
function gameAddCode(code, type) {
    ...
    try {
        ...
        if ((weizhiTypejsh == 'z3_r3_zuhetouzhu' && caizhongTypejsh == '22')
            ...
            var weiShu = 0,codeLen = $wei.attr('length') ;
            ...
            for (var iii = 0; iii < weizhiAr.length; iii++) {
                ...
                //摇一摇任选模式获取位数
                if (typeof code.suiji != "undefined") {
                    if (code.yiyweishu) {
                        code.weiShu = code.yiyweishu;
                    }
                    delete code.suiji;
                    delete code.yiyweishu;
                }
...
function gamePostCode() {
    ...
    // 重置选择的赔率(官方玩法才重置)
    var _type = $('.title_cq.gf_title').find('span').eq(0).text() == '官方' ? 1 : 0;
    if (_type) {
        $.each(code, function (index, value) {
            value.fanDian = gameGetFanDian();
            value.bonusProp = gameGetPl();
        })
    }

    wait();
    var actionNo = $.parseJSON($.ajax('/index/getQiHao/type/' + game.type, {async: false}).responseText);

    destroyWait();
    if (!actionNo.thisNo.actionNo) {
        xingcai('获取投注期号出错');
        return false;
    }
    data.actionNo = actionNo.thisNo.actionNo;
    if ($(':checkbox[name=zhuiHao]')[0].checked) {
        var $dom = $(':checkbox[name=zhuiHao]');
        var zhuiHaostr = $dom.data('zhuiHao');
        var zguihaolist = zhuiHaostr.split(";");
        var str = "";
        var actionNolist = "";
        var beishulist = "";
        var len = zguihaolist.length;
        len = len - 1;
        $.each(zguihaolist, function (i, item) {
            str = zguihaolist[i];
            var strlist = str.split("|");
            if (i == len) {
                actionNolist += strlist[0];
                beishulist += strlist[1];
            } else {
                actionNolist += strlist[0] + '|';
                beishulist += strlist[1] + '|';
            }
        ...
        zhuiHao = 1;
        data.actionNo = actionNolist;
        data.beishu = beishulist;
        //data.zhuiHao=1;
...
function gamePostedCode(err, data) {
    if (err) {
        ...
        reloadMemberInfo();
        gameCalcAmount();
        layer.open({content: "投注成功", btn: "确定"});
        $('#game-tip-dom').text('');
        //reload();
    }
}
...
function gameActionAddCode() {
    //奖金返点限制[如奖金模式在1920以下才能购买分模式(返点大于最大返点-11)] //代理不能买单
    if ($('#wjdl') && parseInt($('#wjdl').val()) > 0) {
        xingcai('代理不能买单');
        return false;
    }
    //var modeName={'2.000':'元','0.200':'角','0.02':'分','0.002':'厘'},
    //var modeName={'1.000':'元'},
    $mode = $('.danwei.dwon'),
...
//alert(userFanDian-fanDian);alert(modeFanDian);
 ...
 * @params value        反点值，可以是个浮点数，表示在当前值时的增量
 ...
 function gameSetFanDian(value){
    ...
    var minPl = parseFloat($dom.data('minpl'));
    var str = minPl + '/' + value.round(1) + '%';
    $dom.data('pl', minPl);
    $dom.text(str);
}
 /**
...
function gameGetFanDian() {
    return parseFloat($('#fandian-value').text().split('-')[1].replace('%', ''));
 ...
 */
function gameGetPl() {
    return $('#fandian-value').text().split('-')[0];
...
function tz5xBDweix() {
    ...
    if ($('#wei-shu :checked', this).length != 3) throw('请选3个位置');
    //if ($('#wei-shu :checked', this).length == 4) len = len * 4;
    //if ($('#wei-shu :checked', this).length == 5) len = len * 10;
    return {actionData: code.split('').join(' '), actionNum: len};
}
...
function tzCombineSelect() {
	...
	var caizhongTypejsh=$('input[name="playedId"]').val();
		...
		$w_len = $('#wei-shu :checked',this).length;
	    if($w_len && $w_len<codeLen)throw('请选'+codeLen+'位数(2)');
		$('#wei-shu :checkbox',this).each(function(i){
			if(this.checked) weiShu.push(i);
		});
...
function ssczx24() {
    var codeLen = parseInt(this.attr('length')), _this = this;
    if($('#wei-shu label input:checked').length != codeLen){
        throw('请选择'+codeLen+'个位置');
    }else{
        return gf4xZx24.call(_this);
    }
...
function ssczx6() {
    var codeLen = parseInt($('#wei-shu').attr('length')), _this = this;
    if($('#wei-shu label input:checked').length != codeLen){
        throw('请选择'+codeLen+'个位置');
    }else{
        return gf4xZx6.call(_this);
    }
    /*var code = [], len = 1, codeLen = parseInt(this.attr('length'));
...
/**任3直选和值*/
function r3_gf3xQ3Zxhz() {
    var _this = this;
    if($('#wei-shu label input:checked').length != $('#wei-shu').attr('length')){
        throw ('请选择'+$('#wei-shu').attr('length')+'个位置');
    }
    return gf3xQ3Zxhz.call(_this);
}
/**任3组选和值*/
function r3_gf3xQ3Zuxhz() {
    var _this = this;
    if($('#wei-shu label input:checked').length != $('#wei-shu').attr('length')){
        throw ('请选择'+$('#wei-shu').attr('length')+'个位置');
    }
    return gf3xQ3Zuxhz.call(_this);
}
/**任2直选和值*/
function r2_gf2xZxhz() {
    var _this = this;
    if($('#wei-shu label input:checked').length != $('#wei-shu').attr('length')){
        throw ('请选择'+$('#wei-shu').attr('length')+'个位置');
    }
    return gf2xZxhz.call(_this);
}
/**任2组选和值*/
function r2_gf2xZuxhz() {
    var _this = this;
    if($('#wei-shu label input:checked').length != $('#wei-shu').attr('length')){
        throw ('请选择'+$('#wei-shu').attr('length')+'个位置');
    }
    return gf2xZuxhz.call(_this);
}
