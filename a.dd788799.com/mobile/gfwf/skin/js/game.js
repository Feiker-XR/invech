...
var T;
var kjTimer;
function gameKanJiangDataC(fptime,kptime){
    //console.log('gameKanJiangDataC');
    $(".current_play_time_left p span").html($("#submenu .tag").html());
    var $dom=$('#count_down');
    var diffTime = 0;
    if(fptime>0){
        diffTime = fptime;
        ...
        $('.tz_change_mask').css("display","none");

    }else{
        diffTime = kptime;
       
        $('.feidanstr').html("距离开奖:");
        $('.tz_change_mask').css("display","block");
    }
    ...
    if(diffTime<0){
        ...
        if(T) clearTimeout(T);
         ...
         kptime --;
        T=setTimeout(gameKanJiangDataC, 1000, fptime,kptime);
    }
}
function getQiHao(){

    $.getJSON('/game/getNo/type/'+game.type, function(data){
            var fptimes =parseInt(data.endtime);
            var kptimes =parseInt(data.opentime);
            var str = data.actionNo;
            ...
            $.each(data.list, function(i,val){ 
                    html += "<div>";
                    html += "<b>第</b>";
                    html += "<span>"+val.number+"</span>";
                    html += "<b>期&nbsp;&nbsp;&nbsp;</b>";
                    html += "<em>";
                    var data = val.data;
                    $.each(data.replace(new RegExp(",","g"), ""), function(j,v){ 
                    
                        html += '<i class="lottery_record_i">'+v+'</i>';
            ...
            $("#histokj").html(html);
            if(T) clearTimeout(T);
            gameKanJiangDataC(fptimes,kptimes);
