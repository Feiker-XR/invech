...
// 默认玩法
handlePlay(_type,_played.id,true);
...
function chooseToPlay(plays){
    ...
    $.each(_group[plays].playeds,function (index,value) {
        if(index != 'tags'){
            if(_played.id == index){
                _li += "<li class='active' onclick='handlePlay("+_type+ "," +value.id+")'>"+ value.name+"</li>"
            }else{
                _li += "<li onclick='handlePlay("+_type+ "," +value.id+")'>"+ value.name+"</li>"
            }
        }
    });
...
//位数变化
$('#wei-shu input:checkbox').live('change',function () {
    if($(this).attr('checked') == 'checked'){
