...
function gamePostCode() {
	...
	wait();
	var actionNo=$.parseJSON($.ajax('/launch.php/game/getNo/type/'+game.type,{async:false}).responseText);
	destroyWait();
	if(!actionNo){
		...
		"确定购买": function() {
			...
			$.ajax('/launch.php/game/postCode', {
				...
				success:function(data, textStatus, xhr){
					if(data.code==0)//失败
					{
						gamePostedCode(data.msg);
					}
					else
					{
						gamePostedCode(null, data.msg);
						if(data) winjinAlert(data.msg,"ok");
					}
...
function gameAddCode (code) {
	wait() ;
	var actionNo = $.parseJSON($.ajax('/launch.php/game/checkBuy',{async:false}).responseText) ;
	destroyWait();
...
function loadKjData(){
	var type=game.type;
	$.ajax('/launch.php/game/getLastKjData/type/'+type,{
		dataType:'json',
		cache:false,
		error:function(){
...
function getQiHao(){
	$.getJSON('/launch.php/game/getQiHao/type/'+game.type, function(data){
