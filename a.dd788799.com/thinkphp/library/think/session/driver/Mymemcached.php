...
//windwos建议使用memcache,linux使用memcached;
//memcache比redis快
class Mymemcached extends SessionHandler
{
    ...
    public function write($sessID, $sessData)
    {
        ...
        $ret = db('session')->insert([
            'session_id' => $sessID,
            'session_expire' => time() + $this->config['expire'],
    ...
    {
        db('session')->where('session_id',$sessID)->delete();        
    ...
    {
        db('session')->where('session_expire','<',time())->delete();   
        return true;
    }
}
