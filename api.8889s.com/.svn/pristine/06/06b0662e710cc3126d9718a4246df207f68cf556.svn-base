...
include_once("../../Member/class/bet.php");

session_start();
if(intval(date('H'))<4)
{
	if($_SESSION["pqauto"]=="0")
	{
...
$sql="select MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR,Match_Master,match_name,Match_Guest,Match_ID from volleyball_match where `Match_Date` ='$mDate' and (MB_Inball>=0 or (MB_Inball='-1' and TG_Inball='-1')) and match_js!='1' order by ID";
...
if($bool){
	$mysqlis->query("update volleyball_match set match_js='1' where match_id in($mid)");
	include_once("../../class/admin.php");
	admin::insert_log($_SESSION["adminid"],"批量审核了排球赛事".$mid."注单");
	$m=count($mids);
}else{
}
}
