    ...
    public function qiandao()
    {
        ...
        if(1/*$this->request->isAjax() && $param['ajax']*/){
            ...
            if(empty($userinfo)){
                ...
                $start_Time_america = date('Y-m-d',time()-12*3600) . " 00:00:00";
                //$start_Time_america = date("Y-m-d",time()-13*3600);
                $end_Time_america = date('Y-m-d',time()-12*3600) . " 23:59:59";
                $liushui_today = 0;
                ...
                $where = array(
                    'username'  => ['=',$username],
                    'betTime'   => ['>=',$start_Time_america],
                    'betTime'   => ['<=',$end_Time_america]
                );
                $liushui_today += Db::table('ag_gameresult')->where($where)->sum('validBetAmount');
                ...
                $where = array(
                    'username'      => $username,
                    'WagersDate'    => $start_Time_america,
                    'WagersDate'    => $end_Time_america
                );
                $liushui_today += Db::table('bbin_gameresult')->where($where)->sum('BetAmount');
