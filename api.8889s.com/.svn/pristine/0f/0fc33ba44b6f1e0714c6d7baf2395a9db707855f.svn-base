...
class Xsc extends Command
{
	static protected $lottery = '新疆时时彩';

    protected function configure()
    {
        $this->setName('xsc')->setDescription('结算'.self::$lottery);
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        include_once ROOT_PATH . 'caiji' . DS .'kaijiang' . DS . 'xjssc_suanfa.php';
	    ...
	    foreach ($rows as $row) {
		    ...
		    // 获取开奖号码
		    $rs = db('c_auto_7')->where('qishu',$row['qishu'])->order('id desc')->find();
		    if (! $rs) continue;
		    ...
		    db('c_auto_7')->where('qishu',$row['qishu'])->update(['ok'=>1,]);
