...
*/
        ...
        foreach($data as $key => $row){
            if($row['gid']){
                ...
                //最高梯度限制的比例
                $bl = db('k_user_fanshui_bili')->where('gid',$row['gid'])->where('tz_ag','<=',$tz_ag)->order('px desc')->find();
                if($bl){
                    $data[$key]['bl'] = $bl['ag'];
	...
	public function ag_fs(){
	    ...
	    $bl = db('k_user_fanshui_bili')->where('gid',$user['gid'])->where('tz_ag','<=',$tz_ag)->order('px desc')->find();
	...
	public function ty(){//1:赢,2:输,
        ...
        foreach($data as $key => $row){
            if($row['gid']){
                $tz = $row['tz'];
                //最高梯度限制的比例
                $bl = db('k_user_fanshui_bili')->where('gid',$row['gid'])->where('tz_ty','<=',$tz)->order('px desc')->find();
                if($bl){
                    $data[$key]['bl'] = $bl['ty'];
    ...
    public function ty_fs(){
        ...
        $bl = db('k_user_fanshui_bili')->where('gid',$user['gid'])->where('tz_ty','<=',$tz_ty)->order('px desc')->find();
    ...
    public function tycg(){//1:赢, 2:输
        ...
        foreach($data as $key => $row){
            if($row['ugid']){
                $tz = $row['tz'];
                //最高梯度限制的比例
                $bl = db('k_user_fanshui_bili')->where('gid',$row['ugid'])->where('tz_ty','<=',$tz)->order('px desc')->find();
                if($bl){
                    $data[$key]['bl'] = $bl['ty'];
                    $data[$key]['je'] = round($tz*$bl['ty']/100,2);
                }
    ...
    public function cp(){
        ...
        foreach($data as $key => $row){
            if($row['gid']){
                $tz = $row['tz'];
                //最高梯度限制的比例
                $bl = db('k_user_fanshui_bili')->where('gid',$row['gid'])->where('tz_cp','<=',$tz)->order('px desc')->find();
                if($bl){
                    $data[$key]['bl'] = $bl['cp'];
    ...
    public function cp_fs(){
        ...
        $bl = db('k_user_fanshui_bili')->where('gid',$user['gid'])->where('tz_cp','<=',$tz_cp)->order('px desc')->find();
    ...
    public function mg(){
        ...
        foreach($data as $key => $row){
            if($row['gid']){
                $tz = $row['tz'];
                //最高梯度限制的比例
                $bl = db('k_user_fanshui_bili')->where('gid',$row['gid'])->where('tz_mg','<=',$tz)->order('px desc')->find();
                if($bl){
                    $data[$key]['bl'] = $bl['mg'];
    ...
    public function mg_fs(){
        ...
        $bl = db('k_user_fanshui_bili')->where('gid',$user['gid'])->where('tz_mg','<=',$tz_mg)->order('px desc')->find();
    ...
    public function bb(){
        ...
        foreach($data as $key => $row){
            if($row['gid']){
                $tz = $row['tz'];
                //最高梯度限制的比例
                $bl = db('k_user_fanshui_bili')->where('gid',$row['gid'])->where('tz_bb','<=',$tz)->order('px desc')->find();
                if($bl){
                    $data[$key]['bl'] = $bl['bb'];
    ...
    public function bb_fs(){
        ...
        $bl = db('k_user_fanshui_bili')->where('gid',$user['gid'])->where('tz_bb','<=',$tz_bb)->order('px desc')->find();
    ...
    public function og(){
        ...
        foreach($data as $key => $row){
            if($row['gid']){
                $tz = $row['tz'];
                //最高梯度限制的比例
                $bl = db('k_user_fanshui_bili')->where('gid',$row['gid'])->where('tz_og','<=',$tz)->order('px desc')->find();
                if($bl){
                    $data[$key]['bl'] = $bl['og'];
    ...
    public function og_fs(){
        ...
        $bl = db('k_user_fanshui_bili')->where('gid',$user['gid'])->where('tz_og','<=',$tz_og)->order('px desc')->find();
    ...
    public function sb(){
        ...
        foreach($data as $key => $row){
            if($row['gid']){
                $tz = $row['tz'];
                //最高梯度限制的比例
                $bl = db('k_user_fanshui_bili')->where('gid',$row['gid'])->where('tz_sb','<=',$tz)->order('px desc')->find();
                if($bl){
                    $data[$key]['bl'] = $bl['sb'];
    ...
    public function sb_fs(){
        ...
        $bl = db('k_user_fanshui_bili')->where('gid',$user['gid'])->where('tz_sb','<=',$tz_sb)->order('px desc')->find();
	...
	public function fanshui_bl(){
	    $data = db('k_user_fanshui_bili b')->join('k_group g','b.gid=g.id')
	...
	public function fanshui_bl_del(){
	    $id = input('id/d');
	    $ret = db('k_user_fanshui_bili')->where('id',$id)->delete();
	    if(!$ret){
	...
	public function fanshui_bl_edit(){
	    if($this->request->isGet()){
	        ...
	        $id = input('id/d');	        
	        if($id>0){//编辑
                $bl = db('k_user_fanshui_bili')->where('id',$id)->find();                
	        }else{
	            $bl = ['id'=>'','gid'=>'','px'=>'','tz'=>'',
	        ...
	        if(intval($data['id'])>0){
	            $ret = db('k_user_fanshui_bili')->update($data);	            
	        }else{
	            unset($data['id']);
	            $ret = db('k_user_fanshui_bili')->insert($data);
	        }
	        if($ret === false){
