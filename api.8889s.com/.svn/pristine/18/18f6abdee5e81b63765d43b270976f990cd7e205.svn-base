...
class pay extends Base{
    ...
    */
    ...
    function code_all(){
        ...
        $thirdcode = db('vc_thirdcode')->where('set_configid = '.$setconfigid.' and status = 0')->order('thirdid')->select();
        foreach ($thirdcode as $k=>$v){
            $result[$k]['id'] = $v['id'];
            $result[$k]['setid'] = $v['setid'];
            $result[$k]['set_configid'] = $v['set_configid'];
    ...
    public function pay(){
		...
		if($uid){
		    ...
		    if(config('pay_use_alone')){
		        ...
		        if(!$user){
		            return $this->msg_jump('用户名不存在!', url('index'));
		        }
		        $uid = $user['uid'];
		    }
		...
		if($transamt<$thirdcode['min']){
			...
			return "<script>alert('$str');window.history.go(-1);</script>";		 
		}
