...
var JqueryDialog = {
	...
	//Header背景色
	"cHeaderBackgroundColor"	:	"#e6e6e6",
	//右上角关闭显示文本
	"cCloseText"				:	"<img src=\"/images/tb1.gif\" width='15' height='15' />",
	//鼠标移上去时的提示文字
	"cCloseTitle"				:	"关闭",
	...
	Open:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight){
		JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, true, true, true,1,1);
	},
	...
	Open1:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag){
		JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag,5,10);
	},
	
	Open2:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight){
		JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, true, false, true, false, 0, 300);
	},	
	/// <summary>创建对话框</summary>
	/// <param name="dialogTitle">对话框标题</param>
	/// <param name="iframeSrc">iframe嵌入页面地址</param>
	...
	/// <param name="isCancelButton">是否呈现“取消”按钮</param>
	/// <param name="isDrag">是否支持拖拽</param>
init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isCloseButton, isDrag, isLeft, isTop){
		
		//获取客户端页面宽高
		var _client_width = document.body.clientWidth;
		var _client_height = document.documentElement.scrollHeight;
		iframeWidth = _client_width;
		//create shadow
		...
		var _left = isLeft?isLeft:(_client_width - (iframeWidth + JqueryDialog.cBorderSize * 2 + 5)) / 2;
		//_jd_dialog.css("left", (_left < 0 ? 0 : _left) + document.documentElement.scrollLeft + "px");
		_jd_dialog.css("left","0");
		
		var _top = isTop?isTop:62; //(document.documentElement.clientHeight - (iframeHeight + JqueryDialog.cBorderSize * 2 + 30 + 30 + 5)) / 2;
		//_jd_dialog.css("top", (_top < 0 ? 0 : _top) + document.documentElement.scrollTop + "px");
		if($('#tzid').length>0){
			var m_top = $('#tzid').offset().top;
			if(m_top>300){
				_top = m_top - 150;
			}
		}				
		...
		//header
		_jd_dialog_m.append("<div id='jd_dialog_m_h' style='border-bottom:1px solid #CCC;height:32px;'></div>");
		var _jd_dialog_m_h = $("#jd_dialog_m_h");
		_jd_dialog_m_h.css("background-image", JqueryDialog.cHeaderBackgroundColor);
		...
		_jd_dialog_m_b_1.css("display", "");
		
		//iframe 容器
		_jd_dialog_m_b.append("<div id='jd_dialog_m_b_2'></div>");
		//iframe
		$("#jd_dialog_m_b_2").append('<iframe src="' + iframeSrc + '" scrolling="yes" frameborder="0" width="' + (iframeWidth-10) + '" height="' + iframeHeight + '" id="' + JqueryDialog.jd_iframe + '" name="' + JqueryDialog.jd_iframe + '" />');
		...
		if(isSubmitButton){
			_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_submit' value='"+JqueryDialog.cSubmitText+"' type='button' onclick='JqueryDialog.Ok();' class='btn btn-info' /></span>");
		}
		if(isCancelButton){
			_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_fx' value='反选' type='button' class='btn btn-info' onclick='JqueryDialog.fx();' /></span>");
			
			_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_cancel' value='"+JqueryDialog.cCancelText+"' type='button' onclick='JqueryDialog.cx();' class='btn btn-info' /></span>");
		}
		if(isCloseButton){
			_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_cl' value='"+JqueryDialog.cCloseText2+"' type='button' class='btn btn-info' onclick='JqueryDialog.Close();' /></span>");
		}
		$(parent.document).scrollTop(_top);
		$('#jd_submit').focus();
	...
	/// <remark></remark>
	Ok:function(){
		var frm = $("#"+JqueryDialog.jd_iframe);	
...
var DragAndDrop = function(){
	...
	var dragMouseDownHandler = function(evt){
		...
		if(_dragObj){
			...
			//获取客户端屏幕尺寸
			_clientWidth = 797; //document.body.clientWidth
