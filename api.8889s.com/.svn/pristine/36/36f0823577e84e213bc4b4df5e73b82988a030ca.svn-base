 ...
 * 获取未结算的期数
 * @param string $type<p>
 *     类型必须是:二分彩,五分彩,重庆时时彩,重庆快乐十分,分分彩,
 *     广东快乐十分,广西快乐十分,江苏快3,北京PK拾,香港六合彩,
 *     新疆时时彩,幸运飞艇</p>
 * @param connection $mysqli
 *     mysqli 链接资源
 * @return mixed 
 *     当不在类型里的时候将返回false,成功查询之后返回一个包含了id,和期数的数组
 */
function getWeiJieSuan($type,$mysqli){
    $typArry = array(
        '二分彩',
        '五分彩',
        '重庆时时彩',
    ...
    if(!in_array($type, $typArry)){
        return false;
    }
    $qisql = "select `id`,`qishu` from weijiesuan where `type`='$type';";
    $query = $mysqli->query($qisql);
    ...
    $qistr = @implode(',', $tmp);
    $ids = @implode(',', $id);
    return array('qistr'=>$qistr,'ids'=>$ids);
}
...
function Six_ZhengMaGuoGuang($haoma,$num){
	if(($num=="大" || $num=="小") && Six_DaXiao($haoma)==$num) {return true;}
	else{return false;}
	if(($num=="单" || $num=="双") && Six_DanShuang($haoma)==$num) {return true;}
	else{return false;}
