...
include ($base."/cqklsf_suanfa.php");
$wjs = getWeiJieSuan('重庆快乐十分', $mysqli);
$qistr = $wjs['qistr'];
$ids = $wjs['ids'];
$qishu = explode(',', $qistr);
sleep(8);//byxx
foreach($qishu as $qi){
    echo "开始执行重庆快乐十分第{$qi}期结算!\r\n";
    if(empty($qi))
        continue;
    //$qi 		= $_REQUEST['qi'];
    //获取开奖号码
    $sql		= "select * from c_auto_4 where qishu=".$qi." order by id desc limit 1";
    ...
    $hm[]		= $rs['ball_8'];
    //根据期数读取未结算的注单
    $sql		= "select * from c_bet where type='重庆快乐十分' and js=0 and qishu=".$qi." order by addtime asc";
    $query		= $mysqli->query($sql);
    $sum		= $mysqli->affected_rows;
    while($rows = $query->fetch_array()){
        if(cqklsf_js($rs, $rows)){
    ...
    $delsql = "delete from weijiesuan where qishu = '$qi' and `type` = '重庆快乐十分';";
    $mysqli->query($delsql);
}
