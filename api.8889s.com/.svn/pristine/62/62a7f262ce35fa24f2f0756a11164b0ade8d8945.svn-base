...
class Lotterybet extends Login{
    ...
    const STATIS_ERROR =  1 ; //接口请求成功返回状态码
    const STATUS_SUCCESS = 0 ; //接口请求失败返回状态码

    //错误码定义
    const  ERROR_UNDER_BALANCE = 10001 ; //余额不足
    const ERROR_IS_END         = 10002 ;//已经封盘
    ...
    public function _initialize(){
        ...
        //清空所有POST数据为空的表单
        //$datas = array_filter($this->request->param());
        ...
        if($this->request->action() != 'six'){
            for ($i = 0; $i < count($datas); $i++){
                $this->allmoney += $datas[''.$names[$i].'']; //计算总金额
                // 加入金额判断
                if($datas[''.$names[$i].''] < 0){
                    return $this->error('投注信息错误!');
                }
            }
        }
        ...
        if(isset($_POST['money'])){
            if(floatval($_POST['money'])<=0){
                return $this->error('投注金额有误!');
            }
        }

        $this->datas = $datas;
        $betc = serialize($this->datas);
        //file_put_contents(dirname(__FILE__).'/betlog.txt', date('Y-m-d H:i:s')."\t".$betc."\r\n",FILE_APPEND);        
        $this->names = $names;
        unset($datas);
        unset($names);
    }
    ...
    {
        try {
            $qishu = \app\logic\qishu::cqssc(); //计算期数

            if($qishu==-1){
                throw new \Exception(self::ERROR_IS_END) ; //已经封盘
            }
            require APP_PATH.'../common/ballname/cqssc.php'; //引入
            ...
            for ($i = 0; $i < count($this->datas); $i++){
                ...
                $money	= $this->datas[''.$this->names[$i].'']; //投注金额
            ...
            $this->bet($betinfo); //投注处理
            return $this->api_success() ;

        } catch (\Exception $e) {
            //错误处理
            $error = $this->disposeError($e) ;
            return $this->api_custom($error);
        }
    }
    ...
    public function pk10()
    {
        try {
            ...
            $ret = $this->bet($betinfo) ;
            if($ret['status']){
                return $this->api_success() ;    
            }else{
                return $this->api_error($ret['msg']);
            }
        } catch (\Exception $e) {
            //错误处理
            $error = $this->disposeError($e) ;
        ...
        switch ($class){
            case 8: //正码过关
                ...
                if(count($this->datas)<2){
                    return $this->api_error("正码过关至少选择 2 项！");
                }
            ...
            case 11: //连码
                ...
                switch ($playtype){
                    case 1 ://普通玩法
                        ...
                        switch ($ball_class){
                            ...
                            case 3 ://三全中
                                ...
                                $ws = count($_POST['ball']);
                                if($ws<3 || $ws>10){
                                    return $this->api_error("只能选择 3 - 10 个号码！");
                                }
                                $zz = $ws*($ws-1)*($ws-2)/6;
                                $allmoney = $zz*$money;
                                $qw = 0;
                            ...
                            case 6 ://三全中
                                ...
                                $ws = count($_POST['ball']);
                                if($ws<2 || $ws>10){
                                    return $this->api_error("只能选择 2 - 10 个号码！");
                                }
                                $zz = $ws*($ws-1)/2;
                                $qw = 0;
                                $allmoney = $zz*$money;
                    ...
                    case 3 ://生肖对碰，尾数对碰
                        ...
                        if($ws!=2){
                            return $this->api_error("只能选择 2 组号码！");
                        }
                        if($_POST['ball_'.$class.'']==4){
                            $odds = $oddinfo['oddslist']['ball']['11']['5'];
                        }
                    ...
                    case 4 ://串肖尾数
                        ...
                        if(!$_POST['ball_sx'] || !$_POST['ball_ws']){
                            return $this->api_error("请正确选择生肖与尾数！");
                        }
                    ...
                    case 5 :
                        ...
                        $brr = $this->arrdel($arr,$brr);
                        if(count($arr)<1){
                            return $this->api_error("请选择胆码！");
                        }
                        if(count($brr)<1){
                            return $this->api_error("请选择拖码！");
                        }
                        if($_POST['ball_'.$class.'']==1){
                            $odds	= $oddinfo['oddslist']['ball']['11'][1];
                            if(count($arr)==3){
                        ...
                        if($_POST['ball_'.$class.'']==1){//四全中
                            if(count($arr)<1 || count($arr)>3){
                                return $this->api_error("请选择 1 - 3 个胆码！");
                            }
                            if(count($arr)==3){
                        ...
                        if($_POST['ball_'.$class.'']==2 || $_POST['ball_'.$class.'']==3){//三全中，三中三
                            if(count($arr)<1 || count($arr)>2){
                                return $this->api_error("请选择 1 - 2 个胆码！");
                            }
                            if(count($arr)==2){
                                for($i=0;$i<count($brr);$i++){
            ...
            case 12: //合肖
                ...
                $ws = count($_POST['ball']);
                if($ws<2 || $ws>11){
                    return $this->api_error("只能选择 2 - 11 个生肖！");
                }
                $wanfa = '';
                for( $i=0; $i<$ws-1; $i++){
                    $sx = $_POST['ball'][$i]+$zs;
            ...
            case 14://生肖连，尾数连
                ...
                switch ($ball_class){
                    case 4 ://五连
                        if($ws<5 || $ws>6){
                            return $this->api_error("只能选择 5 - 6 个选项！");
                        }
                    ...
                    case 3 :
                        if($ws<4 || $ws>6){
                            return $this->api_error("只能选择 4 - 6 个选项！");
                        }
                        $zz = $ws*($ws-1)*($ws-2)*($ws-3)/24;
                        $allmoney = $zz*$money;
                        for ( $a = 0 ; $a < $ws - 3 ; $a++ ){
                    ...
                    case 2 :
                        if($ws<3 || $ws>6){
                            return $this->api_error("只能选择 3 - 6 个选项！");
                        }
                        $zz = $ws*($ws-1)*($ws-2)/6;
                        $allmoney = $zz*$money;
                        for ( $a = 0 ; $a < $ws - 2 ; $a++ ){
                    ...
                    case 1 :
                        if($ws<2 || $ws>6){
                            return $this->api_error("只能选择 2 - 6 个选项！");
                        }
                        $zz = $ws*($ws-1)/2;
                        $allmoney = $zz*$money;
                        for ( $a = 0 ; $a < $ws - 1 ; $a++ ){
            ...
            case 15:
                if(is_numeric(!$_POST['money'])){
                    return $this->api_error("参数错误，请重新下注！");
                }
                ...
                switch ($ball_15){
                    case 1 ://五不中
                        if($ws<5 || $ws>10){
                            return $this->api_error("只能选择 5 - 10 个号码！");
                        }
                        $zz = $ws*($ws-1)*($ws-2)*($ws-3)*($ws-4)/120;
                        $allmoney = $zz*$money;
                        for ( $a = 0 ; $a < $ws - 4 ; $a++ ){
                    ...
                    case 2 ://六不中
                        if($ws<6 || $ws>10){
                            return $this->api_error("只能选择 6 - 10 个号码！");
                        }
                    ...
                    case 3 ://七不中
                        if($ws<7 || $ws>10){
                            return $this->api_error("只能选择 7 - 10 个号码！");
                        }
                    ...
                    case 4 ://八不中
                        if($ws<8 || $ws>11){
                            return $this->api_error("只能选择 8 - 11 个号码！");
                        }
                    ...
                    case 5://九不中
                        if($ws<9 || $ws>12){
                            return $this->api_error("只能选择 9 - 12 个号码！");
                        }
                    ...
                    case 6 ://十不中
                        if($ws<10 || $ws>13){
                            return $this->api_error("只能选择 10 - 13 个号码！");
                        }
                    ...
                    case 7://十一不中
                        if($ws<11 || $ws>13){
                            return $this->api_error("只能选择 11 - 13 个号码！");
                        }
                    ...
                    case 8 : //十二不中
                        if($ws<12 || $ws>14){
                            return $this->api_error("只能选择 12 - 14 个号码！");
                        }
     ...
     *  错误处理
     * @param $e  抛出的异常信息
     * @return mixed  返回错误状态码和错误信息
     */
    private function  disposeError($e)
    {
        //错误处理
        $errorCode = $e->getMessage() ;
        $error['status'] = self::STATIS_ERROR ;
        $error['msg']    = '网络延迟' ;
        if ( $errorCode == self::ERROR_UNDER_BALANCE ) {
            $error['status'] = 2 ;  //前端用
            $error['msg'] = '余额不足';
        } elseif ($errorCode == self::ERROR_IS_END) {
            $error['msg'] = '已经封盘,禁止投注' ;
        }
        return $error ;
    }
