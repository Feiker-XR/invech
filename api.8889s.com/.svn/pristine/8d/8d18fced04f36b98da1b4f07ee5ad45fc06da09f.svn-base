...
class Gxsf extends Command
{
	static protected $lottery = '广西快乐十分';

    protected function configure()
    {
        $this->setName('gxsf')->setDescription('结算'.self::$lottery);
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        include_once ROOT_PATH . 'caiji' . DS .'kaijiang' . DS . 'gxklsf_suanfa.php';
	    ...
	    foreach ($rows as $row) {
		    ...
		    // 获取开奖号码
		    $rs = db('c_auto_5')->where('qishu',$row['qishu'])->order('id desc')->find();
		    if (! $rs) continue;
		    ...
		    db('c_auto_5')->where('qishu',$row['qishu'])->update(['ok'=>1,]);
