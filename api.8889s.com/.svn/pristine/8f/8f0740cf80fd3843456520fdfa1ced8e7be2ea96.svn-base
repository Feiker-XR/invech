...
$sql="select MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR,Match_Master,match_name,Match_Guest,Match_ID from tennis_match where `Match_Date` ='$mDate' and (MB_Inball>=0 or (MB_Inball='-1' and TG_Inball='-1')) and match_js!='1' order by ID";
...
if($bool){
	$mysqlis->query("update tennis_match set match_js='1' where match_id in($mid)");
