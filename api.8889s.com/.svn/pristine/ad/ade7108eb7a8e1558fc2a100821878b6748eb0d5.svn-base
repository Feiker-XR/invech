...
class Six extends Login{
    ...
    public function cancel($bid){
        ...
        $bet = db('c_bet')->where('id',$bid)->find();
        if(!$bet){
            $this->error('注单不存在!');
        }
        ...
        $qishu = $bet['qishu'];
        $kjinfo = db('c_auto_0')->where('qishu','eq',$qishu)->find();

        db()->startTrans();
        try{
            //未结算退还本金
            if(!$bet['js']){                
                $change_balance = $bet['money'];
            }

            //已开奖已结算,输的注单直接退本金,赢的注单退本金扣掉所赢金额;
            if($kjinfo && $bet['js']){
                if($this->jisuan($kjinfo,$bet)){
                    $change_balance = $bet['money'] - $bet['win'];//负数
                    if($before_balance + $change_balance < 0){
                        $this->error('用户余额不足以退还所赢金额!');
                    }
                }else{
                    $change_balance = $bet['money'];
                }
            ...
            $ret = db('k_user')->where('uid',$bet['uid'])->setInc('money', $change_balance);

            $log = [];
            $log['m_order']   = 'SIXPJ'.$bet['id'];
            $log['uid']     = $bet['uid'];
            $log['m_value'] = $change_balance;
            $log['q_qian']  = $before_balance;
            $log['h_qian']  = $before_balance + $change_balance;
            $log['status']  = '1';
            $log['m_make_time'] = date('Y-m-d H:i:s');
            $log['about'] = '六合彩取消注单,订单号:'.$bet['did'].',退还金额:'.$change_balance;
            $log['type'] = '500';
            $ret = db('k_money') -> insert($log);

            //删除注单, 优化为软删除;
            db('c_bet')->where('id',$bid)->delete();
