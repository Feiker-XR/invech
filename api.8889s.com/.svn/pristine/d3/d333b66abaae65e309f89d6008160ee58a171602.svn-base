...
//盘口信息
function loadinfo(){
	$.get("/index.php/lottery/odds/gid/xyft/", function(data)
		{
			if(data.opentime>0 && data.endtime > 0){
				...
				alert('当前彩票已经封盘，请稍后再进行下注！\r当前彩票已经封盘，请稍后再进行下注！<br><br>幸运飞艇开盘时间为：09:02 - 23:57');
				return false;
			}
			/*
			if(data.opentime>0){
				...
				$(".bian_td_inp").html("封盘");
				$("#autoinfo").html("已经封盘，请稍后进行投注！");
				$.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>幸运飞艇开盘时间为：09:02 - 23:57', '提示');
				return false;
...
function hm_odds_new(type,obj){
	if(type == 1){
		...
		$("#touzhutables").html(odds_tabs_3);
	}
	else if(type == 4){

		$("#touzhutables").html(odds_tabs_4);
	}
...
//更新开奖号码
function auto(ball){
	$.get("/index.php/lottery/auto/gid/xyft", {ball : ball}, function(data)
		{
...
//              playRing(1);
                auto_new = true;
				var numinfo='';
				numinfo = numinfo+'冠亚军和：<font>'+data.hms[0]+'</font> | <font>'+data.hms[1]+'</font> | <font>'+data.hms[2]+'</font> | 1V10龙虎：<font>'+data.hms[3]+'</font> | 2V9龙虎：<font>'+data.hms[4]+'</font> | 3V8龙虎：<font>'+data.hms[5]+'</font> | 4V7龙虎：<font>'+data.hms[6]+'</font> | 5V6龙虎：<font>'+data.hms[7]+'</font>';
				...
				$('.kick b').each(function(){
					...
					var nu=data.hm[i];
					setTimeout(function(){e.prop('src','/images/lottery/o9/'+nu+'.png');},fun*600);
					i++;
					fun--;
				});
				auto_new = true;
			...
			var auto_top = '<table width="100%" border="0" cellspacing="1" cellpadding="0" class="clbian"><tr class="clbian_tr_title"><td colspan="2"><a href="/openlist/xyft" target="_blank">更多开奖结果</a></td></tr>';
		...
		$.post("/index.php/lottery/changlong.html",{code:'幸运飞艇'},function(data){
		if(data == false){
