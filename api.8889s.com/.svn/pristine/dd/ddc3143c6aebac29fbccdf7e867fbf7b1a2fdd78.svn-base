...
$sql="select MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR,Match_Master,match_name,Match_Guest,Match_ID from bet_match where `Match_Date` ='$mDate' and (MB_Inball>=0 or (MB_Inball='-1' and TG_Inball='-1')) order by ID";
...
while ($rows = $result->fetch_array()) {
			  ...
			  $mb_inball=$rows['MB_Inball'];
			  $tg_inball=$rows['TG_Inball'];	
			  $bool	=	bet::set($bid,$status,$mb_inball,$tg_inball);echo $bid.'--'.$status.'--'.$mb_inball.'--'.$tg_inball;
			  if(!$bool) break;
