...
include $base.'/xjssc_suanfa.php';
$wjs = getWeiJieSuan('新疆时时彩', $mysqli);
$qistr = $wjs['qistr'];
$ids = $wjs['ids'];
$qiarr = explode(',', $qistr);
sleep(8);//byxx
foreach ($qiarr as $qi) {
    ...
    $hm[] = $rs['ball_5'];
    // 根据期数读取未结算的注单
    $sql = "select * from c_bet where type='新疆时时彩' and js=0 and qishu=" . $qi . " order by addtime asc";
    $query = $mysqli->query($sql);
    ...
    $qi = trim($qi);
    $delsql = "delete from weijiesuan where qishu ='$qi' and `type` = '新疆时时彩';";
    $mysqli->query($delsql);
}
