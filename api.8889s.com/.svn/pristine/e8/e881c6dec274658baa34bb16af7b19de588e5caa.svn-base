...
class Gsf extends Command
{
	static protected $lottery = '广东快乐十分';

    protected function configure()
    {
        $this->setName('gsf')->setDescription('结算'.self::$lottery);
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        include_once ROOT_PATH . 'caiji' . DS .'kaijiang' . DS . 'gdklsf_js.php';
	    ...
	    foreach ($rows as $row) {
		    ...
		    // 获取开奖号码
		    $rs = db('c_auto_1')->where('qishu',$row['qishu'])->order('id desc')->find();
		    if (! $rs) continue;
		    ...
		    db('c_auto_1')->where('qishu',$row['qishu'])->update(['ok'=>1,]);
