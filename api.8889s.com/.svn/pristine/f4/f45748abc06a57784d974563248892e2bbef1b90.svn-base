	...
	$.get("/index.php/lottery/odds/gid/csf/", function(data)
		{
			if(data.opentime>0){
				...
				$.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>重庆幸运农场开盘时间为：10:00 - 次日02:00', '提示');
				return false;
...
function endtime(iTime)
{
    ...
    if(iTime==0){
		...
		$('.open_number > img ').each(function(){
			var e=$(this);
			e.prop('src','/lottery/Images/Open_xyncs/x.gif');
			i++;
		});
    }
	if(iTime==60){
	...
	$.get("/index.php/lottery/auto/gid/csf", {ball : ball}, function(data)
		{
			...
			if(auto_new == false || openqihao - data.numbers == 1){
				...
				$('.open_number > img ').each(function(){
					var e=$(this);
					var nu=data.hm[i];
					setTimeout(function(){e.prop('src','/lottery/Images/Open_xyncs/'+nu+'.png');},fun*600);
					i++;
					fun--;
				});
				auto_new = true;
...
function order_btn(){
		$.ajax({
			type:'POST',
			url:'/index.php/lotterybet/cqklsf.html?type=4',
			data: $("#orders").serializeArray(),
			dataType:'json',
			success:function(msg){
				if(msg.status == 1){
					var orderTop = $(window.parent.document).scrollTop();
					$.jBox(msg.text, {title: ' 以下为您刚刚下注成功的注单', buttons: { '关闭': true }, top : orderTop + 200});
					$("#xjssc_show").hide();
					$("#xjssc_mask").hide();
