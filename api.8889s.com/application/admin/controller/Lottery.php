...
class lottery extends Login{
	 ...
	 * @return mixed|string
	 */
	public function ffc_auto(){

	}

	public function f2c_auto(){
	    $param = $this->request->param();
	    $id = $param['qishu'] ?? '0';
	    $page = $param['page'] ?? '1';
	    $action = $param['action'] ?? '';
		
		$rs = $this->AutoSet('c_auto_2fc',$action,$id,'f2c_auto');
		
		$count = Db::table('c_auto_2fc')
		->count();
		$csc = Db::table('c_auto_2fc')
		->order('qishu desc')
		->paginate(20,$count);
		$extral_info = array();
		foreach ($csc as $k => $rows){
		...
		$this->assign('id', $id);
		$this->assign('page', $page);
		return $this->fetch('f2c_auto');
	}

	public function f5c_auto(){
	    $param = $this->request->param();
	    $id = $param['qishu'] ?? '0';
	    $page = $param['page'] ?? '1';
	    $action = $param['action'] ?? '';
		
		$rs = $this->AutoSet('c_auto_5fc',$action,$id,'f5c_auto');
		
		$count = Db::table('c_auto_5fc')
		->count();
		$csc = Db::table('c_auto_5fc')
		->order('qishu desc')
		->paginate(20,$count);
		$extral_info = array();
		foreach ($csc as $k => $rows){
	...
	public function slew(){
		if(request()->isGet()){
			$lotterys = db('gfwf_lottery')->where('is_ffc',1)->select();
			$type = input('type');
			if(!$type){
				$type = $lotterys[0]['id'];
				$lottery = $lotterys[0];
			}else{
				$lottery = db('gfwf_lottery')->where('is_ffc',1)->where('id',$type)->find();
			}
			$this->view->type = $type;
			$this->view->lotterys = $lotterys;
			...
			$win_rate = input('win_rate/d');
			$profit_rate = input('profit_rate/d');
			$data = ['win_rate'=>$win_rate,'profit_rate'=>$profit_rate];

			$ret = db('gfwf_lottery')->where('id',$type)->update($data);
			if($ret !== false){
				message("设置成功！",url('lottery/slew',['type'=>$type]);
			}else{
				message("设置失败！");
			}
		}
