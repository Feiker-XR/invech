...
class Pk10 extends Command
{
	static protected $lottery = '北京PK拾';

    protected function configure()
    {
        $this->setName('pk10')->setDescription('结算'.self::$lottery);
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        include_once ROOT_PATH . 'caiji' . DS .'kaijiang' . DS . 'pk10_suanfa.php';
	    ...
	    foreach ($rows as $row) {
		    ...
		    // 获取开奖号码
		    $rs = db('c_auto_3')->where('qishu',$row['qishu'])->order('id desc')->find();
		    if (! $rs) continue;
		    $row['qishu'] = 663768;
		    // 根据期数读取未结算的注单
		    ...
		    db('c_auto_3')->where('qishu',$row['qishu'])->update(['ok'=>1,]);
