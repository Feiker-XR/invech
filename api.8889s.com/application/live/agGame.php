...
class agGame extends base
{
 ...
 // DES加密钥匙
    ...
    public static function create($userName, $password = 'jpa12344321', $betLimitCode = 'A', $currencyCode = 'CNY', $isSpeed = '', $isDemo = '1')
    {
        ...
        file_put_contents(dirname(__FILE__) . '/ag.log', $fullUrl . "\r\n", FILE_APPEND);
        file_put_contents(dirname(__FILE__) . '/ag.log', $text . "\r\n", FILE_APPEND);
    ...
    public static function transfer ($username, $amount = '0', $transSN = '')
    {
        ...
        if ($matches[1] !== '0') {
            ...
            file_put_contents(dirname(__FILE__) . '/ag.log', $fullUrl2 . "\r\n", FILE_APPEND);
            $result = file_get_contents($fullUrl2);
            file_put_contents(dirname(__FILE__) . '/ag.log', $result . "\r\n", FILE_APPEND);            
...
//以上为旧写法,没有被使用到
    ...
    static public function AGToWithdrawal($username, $amount = '1', $transSN = '')
    {
        ...
        $text = file_get_contents($fullUrl);
        file_put_contents(dirname(__FILE__) . '/ag.log', $text . "\r\n", FILE_APPEND); 
        //<result info="error" msg="not enough credit"/>       
        preg_match('/info="(.*?)"/', $text, $matches);
        if ($matches[1] !== '0') {
            return - 1;
        } else {
