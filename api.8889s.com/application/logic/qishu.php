...
class qishu {
    ...
    public static function gxsf(){
        ...
        if($qs){
            //return date("Y",$lottery_time).(self::BuLings(date("z",$lottery_time)-6)).self::BuLing($qs['qishu']);
            return date("Y",$lottery_time).(self::BuLings(date("z",$lottery_time+86400))).self::BuLing($qs['qishu']);
        }else{
            return -1;
        }
    }
    ...
    public static function f2ssc(){
        date_default_timezone_set('PRC');
        $lottery_time = time();
        $ctime = date("H:i:s");
        $qs = Db::table('c_opentime_2fc') -> where('kaipan','<=',$ctime)->where('fengpan','>=',$ctime) ->limit(1)->find() ;
        if($qs){
            return date("Ymd",$lottery_time).self::BuLings($qs['qishu']);
        }else{
            return -1;
    ...
    public static function f5ssc(){
        date_default_timezone_set('PRC');
        $lottery_time = time();
        $ctime = date("H:i:s");
        $qs = Db::table('c_opentime_5fc') -> where('kaipan','<=',$ctime)->where('fengpan','>=',$ctime) ->limit(1)->find() ;
        if($qs){
            return date("Ymd",$lottery_time).self::BuLings($qs['qishu']);
        }else{
            return -1;
