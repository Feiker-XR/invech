...
function endtime(iTime)
{
    ...
    if(iTime==0){
		...
		$('.open_number > img ').each(function(){
			var e=$(this);
			e.prop('src','images/Open_ssc/x.gif');
			i++;
		});
	...
	$.post("class/auto_2.php", {ball : ball}, function(data)
		{
			...
			if(auto_new == false || openqihao - data.numbers == 1){
				var numinfo='';
				numinfo = numinfo+'总和：<span><font>'+data.hms[0]+'</font></span>&nbsp;<span><font>'+data.hms[1]+'</font></span>&nbsp;<span><font>'+data.hms[2]+'</font></span>&nbsp;&nbsp;斗牛：<span><font>'+data.hms[7]+'</font></span>&nbsp;&nbsp;梭哈：<span><font>'+data.hms[8]+'</font></span>&nbsp;&nbsp;龙虎：<span><font>'+data.hms[3]+'</font></span>&nbsp;&nbsp;组合：<span><font>'+data.hms[4]+'</font></span>&nbsp;<span><font>'+data.hms[5]+'</font></span>&nbsp;<span><font>'+data.hms[6]+'</font></span>';
				$("#autoinfo").html(numinfo);
				var i=0;
				var fun=5;
				$('.open_number > img ').each(function(){
					var e=$(this);
					var nu=data.hm[i];
					setTimeout(function(){e.prop('src','images/Open_ssc/'+nu+'.png');},fun*600);
					i++;
					fun--;
				});
				auto_new = true;
