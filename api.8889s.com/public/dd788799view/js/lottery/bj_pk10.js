...
//盘口信息
function loadinfo(){
	$.get("/index.php/lottery/odds/gid/pk10/", function(data)
		{
			if(data.opentime>0 && data.endtime > 0){
				...
				alert('当前彩票已经封盘，请稍后再进行下注！<br><br>北京PK拾开盘时间为：09:02 - 23:57');
				return false;
...
function endtime(iTime,endtime)
{
    ...
    if(iTime==0){
		...
		$('.kick b').each(function(n,m){
			...
			var nu=n+1;
			e.prop('src','/images/lottery/open_3/x'+nu+'.png');
			i++;
		});
    }
	if(fengpansTime==0){
...
//更新开奖号码
function auto(ball){
	$.get("/index.php/lottery/auto/gid/pk10", {ball : ball}, function(data)
		{
...
//              playRing(1);
				...
				$('.kick b').each(function(){
					...
					var nu=data.hm[i];
					setTimeout(function(){e.prop('src','/images/lottery/open_3/'+nu+'.png');},fun*600);
					i++;
					fun--;
				});
				auto_new = true;
			...
			var auto_top = '<table width="100%" border="0" cellspacing="1" cellpadding="0" class="clbian"><tr class="clbian_tr_title"><td colspan="2"><a href="/openlist/bjpk10" target="_blank">更多开奖结果</a></td></tr>';
		...
		$.post("/index.php/lottery/changlong.html",{code:'北京PK拾'},function(data){
		if(data == false){
