...
//盘口信息
function loadinfo(){
	$.get("/index.php/lottery/odds/gid/csf/", function(data)
		{
			if(data.opentime>0 && data.endtime > 0){
				...
				alert('当前彩票已经封盘，请稍后再进行下注！<br><br>重庆快乐十分开盘时间为：10:00 - 次日02:00');
				return false;
...
//更新开奖号码
function auto(ball){
	$.get("/index.php/lottery/auto/gid/csf", {ball : ball}, function(data)
		{
...
//              playRing(1);
			...
			var auto_top = '<table width="100%" border="0" cellspacing="1" cellpadding="0" class="clbian"><tr class="clbian_tr_title"><td colspan="2"><a href="/openlist/cqklsf" target="_blank">更多开奖结果</a></td></tr>';
			//data.hmlist.reverse();
			//alert(data.hmlist[0]);
...
$auto_top.sort(function(a,b){return b[0] - a[0];});
			...
			if(i==4) break;
				//alert(key);
				//auto_top = auto_top+'<tr class="clbian_tr_txt" style="'+( i%2==1 ? 'background:#efefef;' : '' )+'"><td class="qihao">'+$auto_top[i][0]+'</td><td class="haoma">'+$auto_top[i][1]+'</td></tr>'
		...
		$.post("/index.php/lottery/changlong.html",{code:'重庆快乐十分'},function(data){
		if(data == false){
