...
var JqueryDialog = {
	...
	//Header背景色
	"cHeaderBackgroundColor"	:	"#5d370b",
	//右上角关闭显示文本
	"cCloseText"				:	"<img src=\"/images/tb1.gif\" />",
	//鼠标移上去时的提示文字
	"cCloseTitle"				:	"关闭",
	...
	init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag){
		...
		var _top = 62; //(document.documentElement.clientHeight - (iframeHeight + JqueryDialog.cBorderSize * 2 + 30 + 30 + 5)) / 2;
...
*/
		...
		_jd_dialog_m.css("border", "#5d370b" + " " + JqueryDialog.cBorderSize + "px solid");
		...
		_jd_dialog_m_h.css("background", JqueryDialog.cHeaderBackgroundColor);
		...
		//iframe
		$("#jd_dialog_m_b_2").append("<iframe id='jd_iframe' name='jd_iframe' src='"+iframeSrc+"' scrolling='no' frameborder='0' width='"+iframeWidth+"' height='"+iframeHeight+"' />");
		...
		if(isSubmitButton){
			_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_submit' value='"+JqueryDialog.cSubmitText+"' type='button' onclick='JqueryDialog.Ok();' class='button1' /></span>");
		}
		_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_fx' value='反选' type='button' class='button1' onclick='JqueryDialog.fx();' /></span>");
		if(isCancelButton){
			_jd_dialog_m_t.append("<span class='jd_dialog_m_t_s'><input id='jd_cancel' value='"+JqueryDialog.cCancelText+"' type='button' onclick='JqueryDialog.cx();' class='button1' /></span>");
		}
