...
function gamePostCode(){
	...
	$.ajax('/index.php/lotterygfwf/postAll', {		
		data:{
			code:code,
			para:data,
			zhuiHao:zhuiHao
...
function gameFreshOrdered(err, msg){
	if(err){
		...
		$('#order-history').load('/index.php/lotterygfwf/getOrdered');
	}
}
/**
 * 加载历史开奖数据
...
function wait(){
	var html = '<img src="' + '/gfwf/images/game/wait.gif" />';
...
function gameSetPl(value, flag, fanDianBdw){
	...
	if(fanDianBdw){
		...
		*/
		var base = parseFloat(value.bonusPropBase);
		var fan2 = base.toFixed(2) + "-" + "0.0%";
		$dom.html("<option value="+fan2+">"+fan2+"</option>");
		$('#lt_sel_dyprize_text').text(fan2);
	}
}
...
function loadKjData(){
	var type=game.type;
	$.ajax('/index.php/lotterygfwf/getLastKjData/type/'+type,{
		dataType:'json',
		cache:false,
		error:function(){
...
function getQiHao(){
	...
	$.getJSON('/index.php/lotterygfwf/getQiHao/type/'+game.type, function(data){
		if(data && data.lastNo && data.thisNo){
			...
			gameKanJiangDataC(data.diffTime, data.thisNo.actionNo);
		}
	});
}
...
function futureNo(action,type){
	...
	if(action.length<=8){
		cur = action.substr(0,8);
	}else{
		cur = action.substr(0,8) + '-' + action.substr(8,3);
	}
	$('#lt_sel_modes_cc_cont').text(cur);	
}
...
function selectGroup(self){
	...
	$('#played-span').load(url,function(){
		...
		$('#tabbar-div-s3').children('li').first().find('.tz_title').addClass('on');
...
function selectPlayed(self){
	...
	$('#played-content').load(url,function(re){
		...
		$(self).siblings('.tz_title').addClass('on');
		$(self).parent('.tz_li').siblings('.tz_li').find('.tz_title').removeClass('on');
		$(self).siblings('div').removeClass('act').addClass('back');
		$old.removeClass('act').addClass('back');
		var a = re;
		if ($(self).children('li').first().find('.tz_title').hasClass('on')) {
			$('#tabbar-div-s3').children('li').first().find('.tz_title').addClass('on');
		}
	});
