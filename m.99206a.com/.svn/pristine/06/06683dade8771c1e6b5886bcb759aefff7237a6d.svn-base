...
$('input[type=checkbox]').click(function() {
	...
	if($('input:radio[name="ball_13"]:checked').val()==null){
		...
		{ $.jBox.tip('请先选择分类，在选择生肖！');return false;}
	}
	...
	if ($("input[name='ball[]']:checked").length > num) {
		...
		{ $.jBox.tip('您最多可以选择'+ num +'个生肖！');return false;}
	} else {
...
function order(){
	...
	if (m <= 0) { $.jBox.tip('请输入下注金额！');return false;}
	if($('input:radio[name="ball_13"]:checked').val()==null){
		//{layer.msg('请先选择分类，在选择生肖！', 2, 3);return false;}
		{ $.jBox.tip('请先选择分类，在选择生肖！');return false;}
	}
	if ($("input[name='ball[]']:checked").length < hm){
		...
		{ $.jBox.tip('请至少选择 '+ hm +' 个生肖！');return false;}
	}
	var checked = [];
	$("input[name='ball[]']:checked").each(function() {
		checked.push($(this).val());
    ...
    $("#xjssc_consider").click(function(){
        $.ajax({
            type: "POST",
            url:"/lotterybet/six.html?type=0&class=13",
            data:$('#orders').serialize(),// 你的formid
