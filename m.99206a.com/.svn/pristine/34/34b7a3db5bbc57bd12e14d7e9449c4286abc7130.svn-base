...
$urlx = base64_encode("http://www.1396cp.com/jsk3/kaijiang/");
...
if($m){
	...
	if($m1){
		...
		foreach ($m1[0] as $v) {
			...
			$preg_nums = "#<td[^>]*class=\"nums\"[^>]*>.*[^>]*<i[^>]*class='lot-no([0-9]+)'></i>[^>]*<i[^>]*class='lot-no([0-9]+)'></i>[^>]*<i[^>]*class='lot-no([0-9]+)'></i>[^>]*</td>#";
			...
			if($m3 && $m4 && $m5){
				...
				$data[] = array(
					'qi'=>substr(trim(str_replace("-", '', $m3[1])), 2,9),
