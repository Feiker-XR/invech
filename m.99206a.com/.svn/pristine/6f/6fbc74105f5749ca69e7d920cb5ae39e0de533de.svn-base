...
//for($pages=0;$pages<$t_page;$pages++){
	$data=theif_data2($webdb["datesite"],$webdb["cookie"],'BK','all',$langx,$pages);
		...
		for($i=0;$i<$cou;$i++){
			...
			if($datainfo[0]+0!=0){
				...
				if($mysqlis->affected_rows){ //有数据，更新
					if($match_type==1){
						$sql	=	"update lq_match set Match_Date='$time[0]',Match_Time='$time[1]',Match_IsLose=$isLose,Match_Type=$match_type,Match_ShowType='$datainfo[7]',Match_Ho='$datainfo[9]',Match_Ao='$datainfo[10]',Match_RGG='$datainfo[8]',Match_DxGG='$datainfo[11]',Match_DxDpl='$datainfo[14]',Match_DxXpl='$datainfo[13]',Match_DsDpl='$datainfo[17]',Match_DsSpl='$datainfo[18]',Match_CoverDate='$CoverDate',Match_MasterID='$datainfo[3]',Match_GuestID='$datainfo[4]',Match_LstTime=now(),iPage=".($pages+1).",iSn=".($i+1)." WHERE Match_ID=$datainfo[0]  AND Match_StopUpdateds=0;";
					}else{
