...
class User1 extends Login{
                ...
                try {
                    ...
                    //NT
                        ...
                        */
                        ...
                        $auth = mgGame1::authenticate();
                        $auth = $auth['body']['access_token'];
                        $info = mgGame1::getAccountByExtref($auth, $account_ext_ref);
                        if(!$info['success']){ 
                            $salt = mycrypt::generate_password(4);
                            $password = mycrypt::generate_password(8);
                            $code = mycrypt::encrypt($password, $salt);
                            $info = mgGame1::createMember($auth, $account_ext_ref, $password,$account_ext_ref);
                            if($info['success']){
                        ...
                        $trans_result = mgGame1::createTranscation($auth,$zz_money,$billno,0,$account_ext_ref, $account_id);
                ...
                try {
                    ...
                    if (24 == $zz_type) {
                        ...
                        */
                        ...
                        $trans_result = mgGame1::createTranscation($auth, $zz_money, $billno, 1, $account_ext_ref, $account_id);
    ...
    public function balance($type){//ajax余额查询
        ...
        switch ($zrtype){
            ...
            case 'mgzr':
            	...
            	@mgGame1::createMember($auth,$mg_username,$password,$account_ext_ref);
                $mgRet = @mgGame1::getBalance($auth,$account_ext_ref);
    ...
    public function report(){ //历史记录
        ...
        if($type == 2){
            $where = [];
            $list = Db::name("live_data")->where($where)->limit(10)->select();
            var_dump($list);die;
        }
    ...
    public function checkUsername($type,$username,$uid){
        //vonly 先查本地用户是否存在，不存在则创建，在查第三方用户是否存在
        $userData = db('k_userlive')->where('username', $username)->select();
        if (empty($userData)) {
            $salt = mycrypt::generate_password(4);
            $password = mycrypt::generate_password(8);
            $code = mycrypt::encrypt($password, $salt);
