...
class money extends Login{
	...
	public function huikuan_api(){ 
	    ...
	    $list = array();
	    $types = array("bank"=>"银行转账","alipay"=>"支付宝","wechat"=>"微信","qq"=>"QQ");
	    foreach ($cards as $v){
            $v['type'] = isset($types[$v['type']])?$types[$v['type']]:"未分类";
	...
	public function huikuan(){
		...
		$stime = $request->param('stime') ? $request->param('stime') : date("Y-m-d",time()).' 00:00:00';

		$etime = $request->param('etime') ? $request->param('etime') : date("Y-m-d",time()).' 23:59:59';
	...
	public function tikuan(){
        ...
        $start_time = !empty($request['start_time'])?$request['start_time'].' 00:00:00':date("Y-m-d",time()).' 00:00:00';
        $end_time   = !empty($request['end_time'])?$request['end_time']." 23:59:59":date("Y-m-d",time()).' 23:59:59' ;
        ...
        $where_order = $where_type = $where_time = $where_status = '';
        $where_username = 'ku.pay_name';
        $wLike = 'not like';
        if (!empty($status)&&$status!=3||$status==0){
            $where_status = "status = $status";
        }
        $where_type = "km.type in (11,255,900)";
        if (!empty($orders)){
            $where_order = "km.$orders desc";
        }
        if (!empty($start_time)||!empty($end_time)){
            $where_time = "km.m_make_time >= '$start_time' and km.m_make_time <= '$end_time'";
        }
        if (!empty($username)){
            $where_username = 'ku.pay_name';
            $wLike = 'like';
...
//        $userId = db('k_money')->where($where_key)
//        $money = db('k_money')->where($where_key)->where($where_type)->where($where_time)->getLastSql();
        $money = db('k_money')
                    ->alias('km')
                    ->join('k_user ku','ku.uid = km.uid')
                    ->where($where_username,$wLike,$username)
                    ->where($where_status)
                    ->where($where_type)
                    ->where($where_time)
                    ->order($where_order)
        ...
        $page = $money->render();
        $sum = Db::view('k_money a','m_id,m_order,m_value,sxf,m_make_time,about,status')
            ->view('k_user b',['pay_name','username'],'a.uid = b.uid ')->where($where_status)->sum('a.m_value');
        $sum =  round($sum,2);
        $sumzsjr = Db::view('k_money a','m_id,m_order,m_value,sxf,m_make_time,about,status')
            ->view('k_user b',['pay_name','username'],'a.uid = b.uid ')->where($where_status)->sum('a.sxf');
        $sumzsjr =  round($sumzsjr,2);
        ...
        $this->assign('username',str_replace("%","",$username));
