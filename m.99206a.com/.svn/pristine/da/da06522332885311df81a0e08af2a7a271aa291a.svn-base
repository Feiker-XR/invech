...
function action(){
    ...
    $.getJSON("/main/systopdao.html?callback=?",function(json){
		if(json.logout > 0){
			...
			var sum = json.sum;
			var html = "";
			html += "<a href='/user/index/mtype/online' target='mainFrame' >在线（"+json.online+"）人</a>";
			html += "<a href='/Money/TiKuan/type/1/status/2' target='mainFrame' onclick=\"stopplay();\">提款（"+json.tknum+"）条</a>";
			html += "<a href='/Money/HuiKuan/status/0' target='mainFrame' onclick=\"stopplay();\">汇款（"+json.hknum+"）条</a>";
