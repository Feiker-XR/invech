...
//盘口信息
function loadinfo(){
	$.get("/lottery/odds/gid/gxsf/", function(data)
		{
			if(data.opentime>0 && data.endtime > 0){
				...
				alert('当前彩票已经封盘，请稍后再进行下注！<br><br>广西快乐十分开盘时间为：09:00 - 21:00');
				return false;
...
//更新开奖号码
/*function auto(ball){
	$.get("/lottery/auto/gid/gxsf", {ball : ball}, function(data)
		{
...
//              playRing(1);
			...
			var auto_top = '<table width="100%" border="0" cellspacing="1" cellpadding="0" class="clbian"><tr class="clbian_tr_title"><td colspan="2"><a href="/openlist/gxklsf" target="_blank">更多开奖结果</a></td></tr>';
			//data.hmlist.reverse();
			//alert(data.hmlist[0]);
		...
		$.post("/lottery/changlong.html",{code:'广西快乐十分'},function(data){
		if(data == false){
...
function order_btn(){
    $.ajax({
        type:'POST',
        url:'/lotterybet/gxsf.html?type=2',
