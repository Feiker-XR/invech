...
class Agent extends Login
{
    ...
    public function jiesuan()
    {
        ...
        $tmp = Db::query($sql);
        */
        $query = db('k_user')->where('is_daili',1)->order('login_time desc');
        ...
        foreach ($tmp as $k => $ag){
            ...
            $syzj = round($tysy-$tytz + $cpsy-$cptz + $cpsy1-$cptz1 + $zrsy,2); 
            //输赢总计 添加真人输赢
            $info[$k]['syzj'] = $syzj;
            $syzj_cp = round($cpsy-$cptz + $cpsy1-$cptz1,2);
            //$info[$k]['yjbl'] = $this->fenchengbili($yxhy,-$syzj,0,-$syzj_cp,round($hyfs,2),round($sxf,2),round($qtfy,2),1);//佣金比例
            $bili = $this->get_bili($yxhy,-$syzj);
            $info[$k]['yjbl'] = '体育：<font color="#FF0000">' . $bili[0] . '%</font> 真人：<font color="#FF0000">' . $bili[1] . '%</font> 彩票：<font color="#FF0000">' . $bili[2] . '%</font>';
            ...
            $yongjin = (($tysy-$tytz) * $bili[0] / 100) + ($syzj_cp * $bili[1] / 100) + ($zrsy * $bili[2] / 100) ;
    ...
    private function get_bili($yxhy, $syzj){
        ...
        /*
        if (($yxhy == $ren['1'] || $syzj == $win['2']) ) {
            return [$ty[1],$cp[1],$zr[1]];
        }
        */
