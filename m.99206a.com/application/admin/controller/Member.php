...
class member extends Login{
	...
	public function ti($uid){
	    ...
	    DB::table('k_user_login')->where('uid','eq',$uid)->update(array('is_login'=>0));
	...
	public function hecha(){
	    ...
	    if($username){
	        ...
	        if($rows){
				...
				$sql_m		=	"select sum(case when type=1 or type = 100 then m_value else 0 end) as ck,sum(case when type=2000 then m_value else 0 end) as zs,sum(case when type=3 or type = 600 then m_value else 0 end) as fs,sum(case when type=4000 then m_value else 0 end) as qt,sum(case when type=11 or type=900 then m_value else 0 end) as qk,sum(case when type=120  then m_value else 0 end) as kk from k_money where uid=".$rows["uid"]." and status=1";
				*/
	            ...
	            */
	            //11不是平台转账,12是平台转账;
	            /*
	            $sql_m		=	"select sum(case when type=1 or type = 100 then m_value else 0 end) as ck,sum(case when type=2 or type=200 or type=2000 then m_value else 0 end) as zs,sum(case when type=3 or type=600 then m_value else 0 end) as fs,sum(case when type=4 or type=4000 then m_value else 0 end) as qt,sum(case when type=11 or type=255 or type=900 then m_value else 0 end) as qk,sum(case when type=120 then m_value else 0 end) as kk from k_money where uid=".$rows["uid"]." and status=1";	            
	            ...
	            */
		        ...
		        $rs_m = db('k_money')->field($field)
		        ->where('uid',$rows["uid"])->where('status',1)->find();
	            ...
	            $qukuan		=	round($rs_m['qk'],2);
	            $koukuan	=	round($rs_m['kk'],2);
	            ...
	            $sql = "select * from k_money where status=1 and uid='$uid' and m_value<0 and type in (11,255,900)";	            
	            $tmprows2 = Db::query($sql);
	            foreach ($tmprows2 as $rows2){
	                $temp = array();
	                $temp['time'] = $rows2['m_make_time'];
	            ...
	            $hechaArr[] = array('keyname'=>'会员账号:','keyvalue'=>$rows["username"]
	            	/*
	            	.'&nbsp;&nbsp;'.'<input onclick="location.href=\'/zhudan/c_bet_search.php?find=1&username='.$rows["username"].'\'" style="color:red" type="button" class="" value="会员异常注单核查" />'
					*/
	            	);
	            /*
				...
				红利赠送 即 公司入款赠送
				*/				
				$hechaArr[] = array('keyname'=>'存款总计:','keyvalue'=>round($rs_m['ck'],2));
				$hechaArr[] = array('keyname'=>'汇款总计:','keyvalue'=>round($rs_h['hk'],2));

	            $hechaArr[] = array('keyname'=>'公司入款赠送总计:','keyvalue'=>$cunkuan3);
	            $hechaArr[] = array('keyname'=>'取款总计:','keyvalue'=>$qukuan);
	            $hechaArr[] = array('keyname'=>'扣款总计:','keyvalue'=>$koukuan);
	            ...
	            //$lye = round($cunkuan+$zengsong+$qukuan+$tiyusy+$tiyucg+$caipiaosy+$caipiaosy1+$adjustmoney-$tiyusys-$tiyucgs-$caipiaosys-$caipiaosys1-$caipiaozh1+$zhuanchu-$zhuanru,2);
	            /*
	            echo '$cunkuan='.$cunkuan.'<br>';
	            echo '$zengsong='.$zengsong.'<br>';
	            echo '$qukuan='.$qukuan.'<br>';
	            echo '$tiyusy='.$tiyusy.'<br>';
	            echo '$tiyucg='.$tiyucg.'<br>';
	            echo '$caipiaosy='.$caipiaosy.'<br>';
	            echo '$caipiaosy1='.$caipiaosy1.'<br>';
	            echo '$adjustmoney='.$adjustmoney.'<br>';
	            echo '$tiyusys='.$tiyusys.'<br>';
	            echo '$tiyucgs='.$tiyucgs.'<br>';
	            echo '$caipiaosys='.$caipiaosys.'<br>';
	            echo '$caipiaosys1='.$caipiaosys1.'<br>';
	            echo '$caipiaozh1='.$caipiaozh1.'<br>';
	            echo '$zhuanchu='.$zhuanchu.'<br>';
	            echo '$zhuanru='.$zhuanru.'<br>';
	            echo '+='.($cunkuan+$zengsong+$qukuan+$tiyusy+$tiyucg+$caipiaosy+$caipiaosy1+$adjustmoney+$zhuanchu).'<br>';
	            echo '-='.($tiyusys+$tiyucgs+$caipiaosys+$caipiaosys1+$caipiaozh1+$zhuanru).'<br>';
	            */
	            $lye = round($cunkuan+$zengsong+$qukuan+$tiyusy+$tiyucg+$caipiaosy+$caipiaosy1+$adjustmoney-$tiyusys-$tiyucgs-$caipiaosys-$caipiaosys1-$caipiaozh1+$zhuanchu-$zhuanru,2);
	            $hechaArr[] = array('keyname'=>'理论账户余额:','keyvalue'=>$lye);
