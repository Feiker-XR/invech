...
class Sports extends Login{
    ...
    public function swlogjson($match_id,$time = ''){
        $where = " where match_id = $match_id ";
        if($time){
            $where .= " and  match_lsttime < '$time'";
        }
        $sql = "SELECT * FROM `bet_match_log` $where   GROUP BY match_id, match_ho,match_ao,match_rgg,match_bzm,match_bzg,match_dxgg,match_dxdpl,match_dxxpl,match_bho,match_bao,match_bdpl,match_bxpl,match_bmdy,match_bgdy,match_bhdy order by match_lsttime desc;";
        $res = Db::connect(config('sportdb'))->query($sql);
        return $res;
    }
    
    
    public function swlog($match_id,$time = ''){
        $match_info =  Db::connect(config('sportdb'))->table('bet_match')->where(['Match_ID'=>$match_id])->find();
    ...
    *已结算 之 重新结算全场
    ...
    public function statistics(){
        ...
        $field .= "sum(case when status in(1,2) then bet_money else 0 end) js,sum(fs) ts,count(case when status in (1,2) then 1 else null end) as js_cnt,";
        $field .= "sum(case when status=0 then bet_money else 0 end) wjs,count(case when status=0 then 1 else null end) wjs_cnt,";
        $field .= "sum(bet_money) bet_money,sum(win) win,count(*) cnt, sum(bet_money-win) as profit";
        $where = [];
        $order = input('order');
        if(!$order){
            $order = 'a.uid';
        ...
        $list = Db::name('k_user a')->join('k_bet b','a.uid=b.uid')->field($field)->where($where)->order($order)->group('a.uid')->paginate(20);
        $sum = Db::name('k_bet')->field("sum(bet_money) bet_money,sum(win) win,count(*) cnt, sum(bet_money-win) as profit")->where($where)->find();
