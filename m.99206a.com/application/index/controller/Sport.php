...
class Sport extends Base
{
	...
	*/
    ...
    public function FT_1_0()
    {
        ...
        */
        $zqgq = Cache::get('zqgq',[]) ?? [];
        $zqgqlstime = Cache::get('zqgqlstime') ?? 0;
        $gq_un = Cache::get('zqgq_un') ?? [];
        if(time() - $zqgqlstime > 30 ){
            //file_put_contents(dirname(__FILE__).'/zqgq.log',date("Y-m-d H:i:s"),FILE_APPEND);
        }else{
           // file_put_contents(dirname(__FILE__).'/zqgq_r.log',date("Y-m-d H:i:s")."\t".(time() - $zqgqlstime)."\r\n",FILE_APPEND);
        }
        if(!$gq_un){
