...
class User extends Login{
    ...
    public function get_money(){ //申请提现
		...
		if($_POST){
			...
			$payvalue = trim(sprintf("%.2f", $_POST["pay_value"]));
			Db::startTrans();//开启事务
            ...
            if($user['pay_card']=='' || $user['pay_num']=='' || $user['pay_address']==''){
                ...
                Db::table('k_user')->where('uid','=',$user['uid'])->update($info);
            }
                ...
                try {
                    ...
                    //NT
                        ...
                        */
                        ...
                        if (!self::checkUsername($type,$username,$uid)){
                        ...
                        $userData = ptGamePlayer::info($temp_username);
    ...
    public function balance($type){//ajax余额查询
        ...
        switch ($zrtype){
            ...
            case 'mgzr':
                ...
                /*vonly 不存在用户则添加*/
                self::checkUsername('mg',$username,$uid);
            ...
            case 'ptzr':
                self::checkUsername('pt',$username,$uid);
                $ret = @ptGamePlayer::create($temp_username);
    ...
    public function zr_data_money(){  //转换记录
    	...
    	$zzinfo = Db::table('zz_info')->where(array('uid'=>$uid))->order('id desc')->paginate(15,$num);
    	$page = $zzinfo->render();
    ...
    public function record_ds(){  //游戏记录 体育单式
        ...
        $total= ['bet_money'=>0 ,"win"=>0, "betwin"=>0];
    	foreach ($list as $row){
    	    $total['bet_money'] += $row['bet_money'];
            $total['win'] += $row['win'];
            $total['betwin'] += ($row['win'] - $row['bet_money']);
        }
    ...
    public function record_cp(){ //游戏记录  彩票游戏
        date_default_timezone_set('PRC');
        $begin = date('Y-m-d 00:00:00',strtotime('-3 days'));
    ...
    public function report(){ //历史记录
        $type = input("type/d", 1);
            ...
            if($start = input("start_time")){
                $filter = false;
                $where['betDate'] = ['gt',$start];
                if($end = input("end_time")){
                    $where['betDate'] = ['between', [$start, $end]];
                }
            }else {
                $filter = input("filter", "today");
                if ($filter == "today") {
                    $start = date("Y-m-d");
                    $end = date("Y-m-d");
                    $where['betDate'] = date("Y-m-d");
                } elseif ($filter == "yesterday") {
                    $start = date("Y-m-d", strtotime("-1 day"));
                    $end =date("Y-m-d", strtotime("-1 day"));
                    $where['betDate'] = date("Y-m-d", strtotime("-1 day"));
                } else {
                    ...
                    if ($filter == "tweek") {
                        $start = $now_start;
                        $end =date("Y-m-d", strtotime("$now_start +6 day"));
                        $where['betDate'] = ['EGT', $now_start];
                    } else {
                        $start = date('Y-m-d', strtotime("$now_start - 7 days"));  //上周开始日期
                        $end = date('Y-m-d', strtotime("$now_start - 1 days"));  //上周结束日期
                        $where['betDate'] = ['between', [$start, $end]];
                    }
                }
            }
        if(in_array($type, [1,2,3,4])){
                switch ($type){
                    case 1:$where['gametype']="live";break;
                    case 2:$where['gametype']="dianzi";break;
                    case 3:$where['gametype']="lottery";break;
                    case 4:$where['gametype']="sport";break;
                }
            $where['username'] = "zhy6955";//\session("username");
            $list = Db::name("live_data")->field("betDate,platform,sum(betAmount) betAmount,sum(validBetAmount) validBetAmount,sum(payOff) payOff")->where($where)->group("betDate")->select();
//            echo Db::name("live_data")->getLastSql();
        }else if($type == 5){
            $where['uid'] = "11449";//\session("uid");
            unset($where['betDate']);
            $where['bet_time'] = ['between',[$start, $end]];
            $list = Db::name("k_bet")->field("date(bet_time) as betDate,sum(bet_money) bet_money,sum(win) win")->where($where)->group("betDate")->select();
            foreach($list as &$row){
                $row['platform'] = "皇冠";
                $row['betAmount'] = $row['bet_money'];
                $row['validBetAmount'] = $row['bet_money'];
                $row['payOff'] =$row['win'];
            }
        }else if($type == 6){
            ...
            unset($where['betDate']);
            $where['bet_time'] = ['between',[$start, $end]];
            $list = Db::name("k_bet_cg_group")->field("date(bet_time) as betDate,date(bet_time) as bet_time2,sum(bet_money) bet_money,sum(win) win")->where($where)->group("betDate")->select();
            foreach($list as &$row){
                $row['platform'] = "皇冠";
                $row['betAmount'] = $row['bet_money'];
                $row['validBetAmount'] = $row['bet_money'];
            ...
            $list = Db::name("c_bet")->field("date(addtime) as betDate,sum(money) bet_money,sum(win) win")->where($where)->group("betDate")->select();
            foreach($list as &$row){
                $row['platform'] = "皇冠";
                $row['betAmount'] = $row['bet_money'];
                $row['validBetAmount'] = $row['bet_money'];
        ...
        $total = ["payOff"=>0, "betAmount"=>0, "validBetAmount"=>0 ];
        foreach ($list as $row){
            $total['betAmount'] += $row['betAmount'];
            $total['validBetAmount'] += $row['validBetAmount'];
            $total['payOff'] += $row['payOff'];
        }
    ...
    public function report_date(){
        ...
        if(in_array($type, [1,2,3,4])){
            ...
            $where["betDate"] = $date;
            $list = Db::name("live_data")->where($where)->paginate($size);
        }elseif ($type == 5){
            $where = "date(bet_time)={$date}";
            $list = Db::name("k_bet")->where($where)->paginate($size);
            foreach($list as &$row){
                $row['platform'] = "皇冠";
                $row['betAmount'] = $row['bet_money'];
                $row['validBetAmount'] = $row['bet_money'];
            ...
            $where = "date(bet_time)={$date}";
            $list = Db::name("k_bet_cg_group")->where($where)->paginate($size);
            foreach($list as &$row){
                $row['platform'] = "皇冠";
                $row['betAmount'] = $row['bet_money'];
                $row['validBetAmount'] = $row['bet_money'];
            ...
            $where = "date(addtime)={$date}";
            $list = Db::name("c_bet")->where($where)->paginate($size);
            foreach($list as &$row){
                $row['platform'] = "皇冠";
                $row['betAmount'] = $row['money'];
                $row['validBetAmount'] = $row['money'];
                $row['payOff'] =$row['win'];
            }
        }
        $total = ["payOff"=>0, "betAmount"=>0, "validBetAmount"=>0 ];
    ...
    public function reg_agent(){ //申请代理
    	...
    	if($_POST){
    		...
    		try {
    			...
    			Db::commit();  //事务成功
    			message("感谢您申请为本公司代理，本公司将以最快的速度审核您的申请，您的申请结果将发送到您的站内信中！","/user/agent");
    		}catch(\Exception $e){
    			Db::rollback();  //数据回滚
    			message("由于网络堵塞，本次申请提款失败。\\n请您稍候再试，或联系在线客服。");
    		}
    ...
    public function record_video(){
        $username = \session("username");
        $list = Db::name("live_data")->where("username", $username)->select();
        var_dump($list);
    }
    ...
    public static function checkUsername($type,$username,$uid){
        //vonly 先查本地用户是否存在，不存在则创建，在查第三方用户是否存在
        $userData = db('k_userlive')->where('uid', $uid)->where('platform',$type)->select();
        if (empty($userData)) {
            $salt = mycrypt::generate_password(4);
            $password = mycrypt::generate_password(8);
            $code = mycrypt::encrypt($password, $salt);
            $sql = "insert into k_userlive (uid,username,code,salt,platform) 
                            values
                    ({$uid},'{$username}','{$code}','{$salt}','{$type}')";
            $q = Db::execute($sql);
            if ($q){
                return true;
            }
