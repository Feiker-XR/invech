    ...
    static function getmatch_info($match_id,$point_column="Match_Name",$ball_sort=""){
        ...
        if($ball_sort != "足球滚球"){
            if(in_array(strtolower($point_column),$point)){ //上半场投注
                $sql	=	"select match_name,match_rgg,match_dxgg,match_master,match_guest,match_date,match_time,match_type,Match_CoverDate,Match_NowScore,Match_Hr_ShowType as match_showtype,Match_BRpk as match_rgg,Match_Bdxpk as match_dxgg,Match_HRedCard,Match_GRedCard,$point_column from bet_match where match_id=$match_id limit 1";
            }else{
                $sql	=	"select match_name,match_rgg,match_dxgg,match_master,match_guest,match_date,match_time,match_type,Match_CoverDate,Match_NowScore,match_showtype,match_rgg,match_dxgg,Match_HRedCard,Match_GRedCard,$point_column from bet_match where match_id=$match_id limit 1";
            }
            $rs = $db->query($sql)[0];
            return $rs;
