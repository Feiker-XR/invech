    ...
    public function gxsf($rows){
      ...
      /*  $rows = Db::name('c_auto_5') -> order('qishu desc') ->limit(3) -> select();
        ...
        foreach($rows as $key=>$rs){
            ...
            $rows[$key]['hm']	= implode($hm,'-');  /* $this->BuLing($rs['ball_1']).','.$this->BuLing($rs['ball_2']).','.$this->BuLing($rs['ball_3']).','.$this->BuLing($rs['ball_4']).','.$this->BuLing($rs['ball_5'])*/;
           $rows[$key]['hms'] = $hms; 
        }
         return  $rows;
      /*  $arr = array(
