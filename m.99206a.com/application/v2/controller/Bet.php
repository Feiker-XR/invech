...
//        }
        ...
        if (is_numeric($bet_money)) {
            ...
            if ($bet_money < 50) {
                return $this->go("交易金额不能少于 50 RMB!");
            }
...
//      }
                ...
                try {
                    ...
                    $log['h_qian']  = $balance;
                    $log['status']  = '1';
                    $log['m_make_time'] = date('Y-m-d H:i:s',strtotime('+12 hours'));
                    ...
                    for ($i = 0; $i < $cg_count; $i ++) {
                        ...
                        $bet_time = date('Y-m-d H:i:s',strtotime('+12 hours'));
                        $data[] = [
                            'uid'           => $uid,
                            'gid'           => $cggid,
                            'ball_sort'     => $ball_sort,
                            ...
                            'master'        => $master,
                            'guest'         => $guest,
                            'betDate'       => date("Y-m-d",strtotime('+12 hours')),
                            //'fs'            => 0,//这里需要修改,
                            //'istongji'      => 1,//这里需要修改,
	...
	error_reporting(E_ERROR | E_WARNING | E_PARSE);
        ...
        if ($db_table == "zqgq_match" || $db_table == "lqgq_match") { // 足球滚球、篮球滚球不验证数据库，直接验证缓存文件
            if ($db_table == "zqgq_match") {
                ...
                $zqgq = cache('zqgq');
                $lasttime = cache('zqgqlstime');
                if ((time() - $lasttime) > 10) { // 当时间超过了3秒，则重新采集，否则不采集
