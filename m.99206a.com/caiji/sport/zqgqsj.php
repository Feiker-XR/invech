...
if (sizeof(explode("gamount",$data))>1){
	...
	for($i=1;$i<$cou;$i++){
		...
		if(  $datainfo[9])	$datainfo[9]	+=	0.00;
		else $datainfo[9]					 =	0;
		if(  $datainfo[10])	$datainfo[10]	+=	0.00;
		else $datainfo[10]					 =	0;
		if(  $datainfo[13])	$datainfo[13]	+=	0.00;
		else $datainfo[13]					 =	0;
		if(  $datainfo[14])	$datainfo[14]	+=	0.00;
		else $datainfo[14]					 =	0;
		if(  $datainfo[23])	$datainfo[23]	+=	0.00;
		else $datainfo[23]					 =	0;
		if(  $datainfo[24])	$datainfo[24]	+=	0.00;
		else $datainfo[24]					 =	0;
		if(  $datainfo[27])	$datainfo[27]	+=	0.00;
		else $datainfo[27]					 =	0;
		if(  $datainfo[28])	$datainfo[28]	+=	0.00;
		else $datainfo[28]					 =	0;
		...
		if($mysqlis->affected_rows){ //有数据，更新
		    if($datainfo[1] == '45.5'){
		        $score_h = $datainfo[19];
		        $score_c = $datainfo[18];
		        if(is_numeric($score_h) && is_numeric($score_c)){
		            $sql = "update bet_match set tg_inball_hr='{$score_h}',mb_inball_hr='$score_c'  WHERE `Match_Name` = '{$datainfo[2]}' AND `Match_Master` = '{$datainfo[5]}' AND `Match_Guest` = '{$datainfo[6]}' and Match_Date = '$match_matchtime' ";
		            $mysqlis->query($sql);
		            //file_put_contents(dirname(__FILE__).'/zqsbbf2.log', date('Y-m-d H:i:s')."\t".$sql."\r\n",FILE_APPEND);
		        }
		...
		file_put_contents(dirname(__FILE__).'/zqgqsjsql.log',date("Y-m-d H:i:s")."\t".$sql."\r\n",FILE_APPEND);
		msg_log($sql,'sql');
		if(FALSE === $mysqlis->query($sql)){
		    ...
		    if($datainfo[0] == '3146178'){
		        //file_put_contents(dirname(__FILE__).'/sqllog.txt', date('Y-m-d H:i:s')."\t".$sql."\r\n",FILE_APPEND);
		    }
		    $datainfo[5] = strip_tags($datainfo[5]);
		    $datainfo[6] = strip_tags($datainfo[6]);
