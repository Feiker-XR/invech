...
//盘口信息
function loadinfo(){
	$.get("/lottery/odds/gid/cqssc/", function(data)
		{
			$("#numbers").html($("#open_qihao").html().replace("期",""));
			if(data.opentime>0 && data.endtime > 0){
				...
				alert('当前彩票已经封盘，请稍后再进行下注！<br><br>重庆时时彩开盘时间为：每日09:00 - 次日02:00');
				return false;
...
//更新开奖号码
function auto(ball){
	$.get("/lottery/auto/gid/cqssc", {ball : ball}, function(data)
		{
			//
			var openqihao = $("#open_qihao").html().replace("期","");
			if(auto_new == false || openqihao - data.numbers == 1 || (openqihao.substr(-3)=="001" && data.numbers.substr(-3)=='120')){
				$("#numbers").html(data.numbers);
//              playRing(1);
				var numinfo='';
				numinfo = numinfo+'总和：<font>'+data.hms[0]+'</font> | <font>'+data.hms[1]+'</font> | <font>'+data.hms[2]+'</font> | 龙虎：<font>'+data.hms[3]+'</font> | 前三：<font>'+data.hms[4]+'</font> | 中三：<font>'+data.hms[5]+'</font> | 后三：<font>'+data.hms[6]+'</font>';
				...
				/*$('.kick').each(function(){
					var e=$(this).children('img');
					var nu=data.hm[i];
					setTimeout(function(){e.prop('src','images/open_2/'+nu+'.png');},fun*600);
					i++;
					fun--;
				});*/
			...
			var auto_top = '<table width="100%" border="0" cellspacing="1" cellpadding="0" class="clbian"><tr class="clbian_tr_title"><td colspan="2"><a href="/openlist/cqssc" target="_blank">更多开奖结果</a></td></tr></tr>';
		...
		$.post("/lottery/changlong.html",{code:'重庆时时彩'},function(data){
		if(data == false){
