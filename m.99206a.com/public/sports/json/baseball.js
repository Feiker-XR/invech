...
function loaded(league,thispage,p){
	...
	$.getJSON("/sport/baseball.html?leaguename="+league+"&CurrPage="+thispage+"&callback=?",function(json){
		...
		if(pagecount == 0){
    		...
    		htmls+="<td><div class='rangqiu_odds'><span class='odds'>"+(dbs[i]["Match_DsDpl"]<="0" ? "" :("<a href=\"javascript:void(0)\" title=\"单\" onclick=\"javascript:setbet('棒球单式','单双-单','"+dbs[i]["Match_ID"]+"','Match_DsDpl','0','0','单');\" style='"+(dbs[i]["Match_DsDpl"]!=data[i]["Match_DsDpl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFFF00":"")+"'>"+formatNumber(dbs[i]["Match_DsDpl"],2)+"</a>"))+"</span><span class='pankou'>"+(dbs[i]["Match_DsDpl"]>"0" ? "单" :"")+"</span><br><span class='odds'>"+(dbs[i]["Match_DsDpl"]<="0" ? "" :("<a href=\"javascript:void(0)\" title=\"双\" onclick=\"javascript:setbet('棒球单式','单双-双','"+dbs[i]["Match_ID"]+"','Match_DsSpl','0','0','双');\" style='"+(dbs[i]["Match_DsDpl"]!=data[i]["Match_DsDpl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFFF00":"")+"'>"+formatNumber(dbs[i]["Match_DsSpl"],2)+"</a>"))+"</span><span class='pankou'>"+(dbs[i]["Match_DsSpl"]!="0" ? "双" :"")+"</span></div></td>";
    		htmls+="</tr>";	
			}
			}
...
$(document).ready(function(){
	$("#xzls").click(function(){ //选择联赛
		JqueryDialog.Open('棒球单式', 'json/dialog.php?lsm='+window_lsm, 600, window_hight);
	});
});