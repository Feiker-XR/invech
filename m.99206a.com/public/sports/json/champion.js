...
function loaded(league,thispage,p){
	...
	$.getJSON("/sport/champion.html?leaguename="+league+"&CurrPage="+thispage+"&callback=?",function(json){
		...
		if(window_lsm.length > 2000){
			if(window.XMLHttpRequest){ //Mozilla, Safari, IE7 
				if(!window.ActiveXObject){ // Mozilla, Safari, 
					JqueryDialog.Open('冠军', '/sport/dialog.html?lsm='+window_lsm, 600, window_hight);
				}else{ //IE7
					JqueryDialog.Open('冠军', '/sport/dialog.html?lsm=gj', 600, window_hight);
				}
			}else{ //IE6
				JqueryDialog.Open('冠军', '/sport/dialog.html?lsm=gj', 600, window_hight);
			}
