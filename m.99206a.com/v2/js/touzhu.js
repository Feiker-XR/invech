...
if(location.href.indexOf('touzhutype=1') > -1){
    touzhutype=1
}
var cg_count=0; //串关串数
var time_id='';
var u_m = parseFloat(sessionStorage.getItem('u_m'));
...
function betmoneySports(){
	...
	// alert(bet_money);
	 ...
	 //var user_money = $(top.mem_index.document).find('#user_money').html();
	 var user_money = u_m;
...
////////////////////////////////////////////////////
	...
	if(_url && _url.indexOf('touzhutype=1')>-1){
		$('#touzhutype').val('1');
	}
});
...
$.getJSON("/sport/leftdao.html",function(json){
		//console.log(json);
		// 串关
...
function quxiao_bet(){
	 ...
	 /* $("#maxmsg_div").fadeOut();
      ...
      $('.selected_box>div>span').html('已选(0)场');
      //autofitframe();
}
...
//// 投注(足够三场,显示详情)
$('.selected_box>div>input').click(function(){
	if(parseInt($('.selected_box>div>span').text().split('(')[1].split(')')[0]) <3 ){
		alert('请至少选择三场')
	}else{
		$('#bet_box').height(485);
		$('#bet_box').fadeIn();
...
function del_bet(obj){
	if(touzhutype==0){
		...
		$('.selected_box>div>span').html('已选('+cg_count+')场');
		if(cg_count==1)
		{
		 $("#bet_moneydiv").fadeOut();
		}
	...
	//下注函数
	   clear_input();
 	   ...
 	   if(touzhutype==0){  
			...
			$("#cg_num").html(cg_count);
			$('.selected_box>div>span').html('已选('+cg_count+')场');
			alert('您选择了'+cg_count+'场比赛')
			//$("#cg_msg").show();
		    $("#touzhudiv").append(data);	//.fadeIn()
...
function check_bet_jsc(){
	...
	if($("#istz_jsc").css("display")=="block"){
		...
		$.getJSON(
		    "/sport/checkxe_jsc.html?issue="+issue+"&ball_sort="+ball_sort+"&point_column="+point_column+"&bet_point="+bet_point+"&bet_money="+bet_money+"&numbers="+numbers+"&touzhuxiang="+touzhuxiang+"&callback=?",
			function(json){
				if(json.result == "ok"){
...
function check_bet(){ //提交按钮，提交数据检查
	...
	if($("#istz").css("display")=="block"){
		...
		if(touzhuxiang=="大小"){
			 var match_dxgg=$('input[name="match_dxgg[]"]').val();
			  if(ball_sort.indexOf("足球滚球")>=0){
				  ...
				  //alert(bet_info);y
				  //bet_info=bet_info.match(/-[U|O][0-9.\/]{0,}@/);
				  bet_info=bet_info.match(/[0-9.\/]{0,}@/);
				  //alert(bet_info);
				  bet_info ='-'+bet_info;
				  bet_info = bet_info.substring(1,bet_info.length-1);
				 // alert(bet_info);
				  //alert(match_dxgg);
			  }
			  if(bet_info!=match_dxgg || match_dxgg==176){
				  alert("ERROR601:当前盘口已经改变，请重新下单");
				  $("#submit_from").attr("disabled",false); //按钮有效
				  return false;
			  }
		}
		...
		if(touzhutype==0){
			...
			$.getJSON(
				"/sport/checkxe.html?ball_sort="+ball_sort+"&touzhuxiang="+touzhuxiang+"&bet_money="+bet_money+"&match_id="+match_id,
				function(json){
					if(json.result == "ok"){
						...
						//window.location.href = "left.php";
						window.location.href = "/index/left.html";
					}else if(json.result == "weihu"){
						alert(json.reason);
						$("#submit_from").attr("disabled",false); //按钮有效
						return false;
			...
			if(bet_money>=yinghuo ){
				alert("ERROR-5:赔率已改变，请重新下单");
				$("#submit_from").attr("disabled",false); //按钮有效
				return false;
			}
			$.getJSON(
				"/sport/checkcg.html?bet_money="+bet_money,
				function(json){
					if(json.result == "ok"){
							$("#istz").css("display","block");
							$("#bet_money").removeClass("edit");
...
$('#submit_from').click(function(){
	console.log(111);
    //$('#cg_msg').css('display')!='none'
    if(touzhutype == 1) {
        if (parseInt($('#cg_num').html(),10)>=3) {
            check_bet();
        }else{
            alert('投注失败，请至少选择三场比赛后再进行投注！');return false;
        }
    }else{
        check_bet();
    }
    if($('#submit_from').attr('firsttime')){
        $('#submit_from').removeAttr('firsttime');
        $.ajax({
            url:'/bet/index.html',
            type: 'post',
			dataType:'json',
            ...
            complete:function(){
                $('#submit_from').removeAttr('disabled');
            }
        })
    }else{
        $('#submit_from').attr('firsttime','true');
    }
