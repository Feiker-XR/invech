...
$sql		=	"select id from bet_match where match_date!='".date("m-d")."' and Match_CoverDate>'".date('Y-m-d H:i:s')."' and Match_Hr_Bd10>0 ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from bet_match where match_date!='".date("m-d")."' and Match_CoverDate>'".date('Y-m-d H:i:s')."' and Match_Hr_Bd10>0 group by match_name";
	$query	=	$mysqlis->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
