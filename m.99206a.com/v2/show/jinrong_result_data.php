...
//页数统计
$sql		=	"select x_id,x_title from t_guanjun where match_type=2 and x_result!='' ";
if(@$_GET["leaguename"]<>""){
...
$mysqlis->query($sql.$sqlwhere);
$count_num	=	$mysqlis->affected_rows;
$query		=	$mysqlis->query($sql.$group); 
$i			=	1;
while($i<=$count_num){
	$p_page++;
	$i+=$bk;
...
if($count_num=="0"){
	...
	$sqlwhere	=	" where tg.match_type=2 and tg.x_id=tt.xid and tg.x_result!='' ";
