...
function setGameZhuiHao(data){
	...
	wait();
	var actionNo=$.parseJSON($.ajax('/index.php/lotterygfwf/getNo/type/'+data.type,{async:false}).responseText);
	destroyWait();
	$.get('/lotterygfwf/zhuiHaoModal', function(html){
		// console.log(html)
		$(html).dialog({
			...
			open:function(event, ui){
				...
				str +='actionNum/'+actionNo.actionNo+'/';
				$this.attr('src', '/lotterygfwf/zhuiHaoQs/'+str);
...
function gamePostCode(){
	...
	wait();
	var actionNo=$.parseJSON($.ajax('/index.php/lotterygfwf/getNo/type/'+game.type,{async:false}).responseText);
	destroyWait();
	if(!actionNo){
		xingcai('获取投注期号出错');
		return false;
...
function tzAllSelect(){
	...
	if(this.has('.checked').length!=codeLen){throw('请选'+codeLen+'位数字')}else{
...
//			throw('选择号码不正确');
	...
	code=code.join(',');
	if(code.split(",")[0].length< 1 || code.split(",")[1].length< 3) {
