...
$.getJSON("/index.php/sport/leftdao.html?callback=?",function(json){
					$("#s_zq").html("("+json.zq+")");
...
function quxiao_bet(){
    ...
    clear_input();
	  ...
	  $('#s_betiframe',parent.document).animate({height:0}, 300);
      $("#touzhudiv").html('');
      $("#kefu").fadeIn();
      if($("#touzhutype").val()==1){
          $("#cg_01_bet").show();
...
function check_bet_jsc(){
	...
	if($("#istz_jsc").css("display")=="block"){
		...
		$.getJSON(
		    "/index.php/sport/checkxe_jsc.html?issue="+issue+"&ball_sort="+ball_sort+"&point_column="+point_column+"&bet_point="+bet_point+"&bet_money="+bet_money+"&numbers="+numbers+"&touzhuxiang="+touzhuxiang+"&callback=?",
			function(json){
				if(json.result == "ok"){
...
function check_bet(){ //提交按钮，提交数据检查
	...
	if($("#istz").css("display")=="block"){
		...
		if(touzhutype==0){
			...
			$.getJSON(
				"/index.php/sport/checkxe.html?ball_sort="+ball_sort+"&touzhuxiang="+touzhuxiang+"&bet_money="+bet_money+"&match_id="+match_id,
				function(json){
					if(json.result == "ok"){
						...
						//window.location.href = "left.php";
						window.location.href = "/index.php/index/left.html";
					}else if(json.result == "weihu"){
						alert(json.reason);
						$("#submit_from").attr("disabled",false); //按钮有效
						return false;
			...
			$.getJSON(
				"/index.php/sport/checkcg.html?bet_money="+bet_money,
				function(json){
					if(json.result == "ok"){
							$("#istz").css("display","block");
							$("#bet_money").removeClass("edit");
