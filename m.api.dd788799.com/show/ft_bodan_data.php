...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select match_name from bet_match where Match_Type=1 and Match_IsShowbd=1 and Match_CoverDate>'".date('Y-m-d H:i:s')."' and Match_Bd21>0 group by match_name";
	$query	=	$mysqlis->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
