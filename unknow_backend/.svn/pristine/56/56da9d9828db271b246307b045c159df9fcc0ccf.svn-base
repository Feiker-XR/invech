...
$sql		=	"select x_id from t_guanjun where match_type=2 and match_coverdate>'".date('Y-m-d H:i:s')."' and x_result is null ".$sqlwhere.' order by  match_coverdate,match_name,x_id';
...
if($i == 1){ //未查找到记录
	...
	$sql	=	"select x_title from t_guanjun where match_type=2 and match_coverdate>'".date('Y-m-d H:i:s')."' and x_result is null group by x_title";
	$query	=	$mysqlis->query($sql);
	$i		=	0;
	$lsm	=	'';
	while($row = $query->fetch_array()){
