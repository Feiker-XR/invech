...
function loaded(league,thispage,p){
	...
	$.getJSON("/index.php/sport/FT_1_0.html?leaguename="+league+"&CurrPage="+thispage+"&callback=?",function(json){
		...
		if(pagecount == 0){
			...
			for(var i=0; i<dbs.length; i++){
				...
				htmls+="    <td class='zhong line'>"+temphwin+"<br>"+temphlose+"<br>"+temphdraw+"</td>";
				htmls+="    <td><div class='rangqiu_odds'><span class='odds'>"+(dbs[i]["Match_BHo"]!=null?tempshrgl:"")+"</span><span class='pankou'>" +(dbs[i]["Match_Hr_ShowType"]=="H" && dbs[i]["Match_BAo"] !="0"?tempsrsgl:"") + "</span><br><span class='odds'>"+(dbs[i]["Match_BAo"] !=null?tempsgrgl:"")+"</span><span class='pankou'>" + (dbs[i]["Match_Hr_ShowType"]=="C" && dbs[i]["Match_BAo"] !="0"?tempsrsgl:"")+ "</span><br>&nbsp;</td>";
...
$(document).ready(function(){
	$("#xzls").click(function(){ //选择联赛
		JqueryDialog.Open('足球滚球', '/index.php/sport/dialog.html?lsm='+window_lsm, 600, window_hight);
	});
});