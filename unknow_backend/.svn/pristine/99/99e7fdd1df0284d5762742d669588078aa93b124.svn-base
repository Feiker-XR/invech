...
if (sizeof(explode("gamount",$data))>1){
	...
	try{
	    ...
	    $cjids = '';
    	$tmpids = trim($tmpids,',');
    	$qsql = "select * from lq_match where match_id in ($tmpids)";
    	$query = $mysqlis->query($qsql);
    	if($query){
    	    while($cjrow = $query->fetch_array()){
    	        $cjids .= $cjrow['Match_ID'] . ',';
    	        $count++;
    	    }
    	    if($count != $msg){
    	        file_put_contents(dirname(__FILE__).'/lqgq_error.log', date('Y-m-d H:i:s')."\t采集数据与入库数据不匹配，采集id为{$tmpids},共计{$msg}条，实际入库{$cjids}共{$count}条！\r\n",FILE_APPEND);
    	    }
    	}else{
    	    if( $msg != 0){
    	        file_put_contents(dirname(__FILE__).'/lqgq_error.log', date('Y-m-d H:i:s')."\t sql语句 {$qsql}运行错误！\r\n",FILE_APPEND);
    	    }
    	}
