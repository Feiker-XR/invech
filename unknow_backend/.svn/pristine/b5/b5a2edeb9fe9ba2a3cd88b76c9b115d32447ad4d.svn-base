	...
	$.get("/index.php/lottery/odds/gid/xyft/", function(data)
		{
			if(data.opentime>0){
				...
				$.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>幸运飞艇开盘时间为：13:05 - 次日04:05', '提示');
				return false;
...
function loadinput(ball , s){
    b = "ball_"+ball+"_"+s;
    n = "<input name=\""+b+"\" id=\""+b+"\" class=\"inp1\" onkeyup=\"digitOnly(this)\" onblur=\"this.className='inp1';\" type=\"text\" maxLength=\"7\"/>"
    if (ball==1){
        $("#ball_1_t"+s).html(n);
...
function endtime(iTime)
{
    ...
    if(iTime==0){
		...
		$('.open_number > img ').each(function(){
			var e=$(this);
			e.prop('src','/lottery/Images/Open_pk10/x.gif');
			i++;
		});
    }
	if(iTime==60){
	...
	$.get("/index.php/lottery/auto/gid/xyft", {ball : ball}, function(data)
		{
			...
			if(auto_new == false || openqihao - data.numbers == 1){
				...
				$('.open_number > img ').each(function(){
					...
					setTimeout(function(){
						e.prop('src','/lottery/Images/Open_pk10/'+nu+'.gif');},fun*600);
					i++;
					fun--;
				});
				auto_new = true;
...
function order_btn(){
		$.ajax({
			type:'POST',
			url:'/index.php/lotterybet/xyft.html?type=9',
