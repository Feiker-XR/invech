...
class Pay extends Login{
    ...
    */
    ...
    public function paythird_set(){
        ...
        $m_group = db('k_group')->field(array('id','name'))->select();
        ...
        foreach ($set_config as $k=>$v){
            ...
            $result[$k]['gid'] = $thirdcode['gid'];
            $result[$k]['order'] = $thirdcode['order'];
        }
    ...
    public function order_edit(){
        ...
        if(request()->isGet()){ 
            ...
            if($info['order_state'] == '0' && $order_state == '1'){
                $user = db('k_user')->where(['uid'=>$order['uid']])->find();
                $orderinfo = db('k_money')->where(['m_order'=>$orderid])->find();
                if(!$orderinfo){
                    $money_data = [
                        'uid'           =>  $info['uid'],
                        'm_value'       =>  $info['order_money'],
                        'm_value1'      =>  0,
                        'm_order'       =>  '手动变更',//商户订单号
                        'status'        =>  1,
                        'm_make_time'   =>  date('Y-m-d H:i:s'),
                        'balance'       =>  $user['money']+$info['order_money'],
                        'q_qian'        =>  $user['money'],
                        'h_qian'        =>  $user['money']+$info['order_money'],
                        'type'          => '100',
                        'about'       => $info['pay_api'],
                    ];
                    db('k_money')->insert($money_data);
                    //添加用户账户余额
                    db('k_user')->where(['uid'=>$info['uid']])->setInc('money',$info['order_money']);
                    $log_info = '支付成功且充值成功!订单号：' . $orderid. '订单金额：' . $info['order_money'];
                    sys_log($info['uid'], $log_info);
                }
            }
