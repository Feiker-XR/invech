...
class Sports extends Login{
    ...
    public function live(){
        if($this->request->isPost()){
            ...
            if(empty($params['id'])) {
                $this->success("请勾选要修改的记录！！");
                return false;
            }
            $db = Db::connect('sportdb');
            $counter = 0;
            foreach ($params['id'] as $i=>$id){
                $row['time'] = $params['time'][$i];
                $row['date'] = $params['date'][$i];
                $row['race'] = $params['race'][$i];
                $row['name'] = $params['name'][$i];
                $row['match_master'] = $params['match_master'][$i];
                $row['match_guest'] = $params['match_guest'][$i];
                if($db->name('live')->where('id', $id)->update($row)){
                    $counter++;
                };
            }
            $this->success("成功修改{$counter}条信息！！",'/sports/live.html');
        }else{
            $where['date'] = Input('page_date', date('Y-m-d'));
            $where['status'] = Input('status/d', 0);
            $list = Db::connect('sportdb')->name('live')->where($where)->order('date,time')->paginate(20);
            $where['edit_mode'] = Input('edit_mode/d',0);
            $page = $list->appends($where)->render();
            $this->assign('status', $where['status']);
            $this->assign('list', $list);
            $this->assign('page', $page);
            $this->assign('edit_mode', Input('edit_mode/d',0));
            $this->assign('page_date', $where['date']);
            return $this->fetch();
        }
    }
    ...
    public function lanqiu(){
        ...
        $list = $db->table('lq_match')->where('match_js',$js)
        ->where('match_date',['=',$page_date],['=',$page_date2],'or')
    ...
    public function badminton(){
        ...
        $list = $db->table('badminton_match')->where('match_js',$js)
            ->where('match_date',['=',$page_date],['=',$page_date2],'or')
            ->field('Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest,Match_MasterID,Match_GuestID,Match_Name,MB_Inball_HR,TG_Inball_HR,MB_Inball,TG_Inball,match_js')
    ...
    public function billiards(){
        ...
        $list = $db->table('billiards_match')->where('match_js',$js)
            ->where('match_date',['=',$page_date],['=',$page_date2],'or')
            ->field('Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest,Match_MasterID,Match_GuestID,Match_Name,MB_Inball_HR,TG_Inball_HR,MB_Inball,TG_Inball,match_js')
            ->order('Match_CoverDate,Match_Name,Match_Master desc')
            ->paginate(15);
    ...
    public function other(){
        ...
        $list = $db->table('other_match')->where('match_js',$js)
            ->where('match_date',['=',$page_date],['=',$page_date2],'or')
            ->field('Match_ID, Match_Date, Match_Time, Match_Master, Match_Guest,Match_MasterID,Match_GuestID,Match_Name,MB_Inball_HR,TG_Inball_HR,MB_Inball,TG_Inball,match_js')
            ->order('Match_CoverDate,Match_Name,Match_Master desc')
            ->paginate(15);
    ...
    public function tennis(){
        ...
        $list = $db->table('tennis_match')->where('match_js',$js)
        ->where('match_date',['=',$page_date],['=',$page_date2],'or')
    ...
    public function volleyball(){
        ...
        $list = $db->table('volleyball_match')->where('match_js',$js)
        ->where('match_date',['=',$page_date],['=',$page_date2],'or')
        ->field('ID,Match_ID,Match_Time,Match_Master,Match_Guest,Match_Name,MB_Inball,TG_Inball')
        ->order('Match_CoverDate,Match_Name,Match_Master desc')
        ->paginate(15);
    ...
    public function baseball(){
        ...
        $list = $db->table('baseball_match')->where('match_js',$js)
        ->where('match_date',['=',$page_date],['=',$page_date2],'or')
    ...
    public function guanjun(){
        ...
        if( isset($param['date']) && true ){
            $query->where('match_coverdate','like','%'.$page_date.'%');
        }         
    ...
    */
    public function jiesuan_list(){
        ...
        $mid        =   implode(",",$mid_array);
        //$table = 'bet_match';
        $table = Input('type/s','bet_match');

        //比分
        $db = Db::connect(config('sportdb'));
        $r = $db->table($table)->where('Match_ID','in',$mid)
        ->field('Match_ID,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR')->select();
        $m     		=	array();
        foreach($r as $rows){
            $m[$rows["Match_ID"]]["MB_Inball"] = intval($rows["MB_Inball"]);
            $m[$rows["Match_ID"]]["TG_Inball"] = intval($rows["TG_Inball"]);
                $m[$rows["Match_ID"]]["MB_Inball_HR"] = intval($rows["MB_Inball_HR"]);
                $m[$rows["Match_ID"]]["TG_Inball_HR"] = intval($rows["TG_Inball_HR"]);
        }
        ...
        */
        ...
        foreach ($list as $rows) {
            ...
            $rows["MB_Inball"] = $m[$rows["match_id"]]["MB_Inball"];
            $rows["TG_Inball"] = $m[$rows["match_id"]]["TG_Inball"];
            ...
            $column=$rows["point_column"];
            //$t = make_point(strtolower($column),"","",$rows["MB_Inball"],$rows["TG_Inball"],$rows["match_type"],$rows["match_showtype"],$rows["match_rgg"],$rows["match_dxgg"],$rows["match_nowscore"]);
            if($table == 'lq_match') $rows["match_nowscore"] = '0:0'; //篮球所有的都按最终比分结算，所以将当前比分设为0:0
    ...
    */
    public function do_jiesuan(){
        ...
        //$php    =   $_GET['php'] ? $_GET['php'] : 'ZuQiu';
        $table  =   input('type','bet_match');
        if(empty($table)) $table = 'bet_match';
    ...
    */
    public function nullity_list(){
        ...
        */
        ...
        foreach ($list_cg as $rows) {
            $rows['win']		=	$rows['bet_money']*$rows['bet_point']; //可赢金额=交易金额*当前水位
            if(in_array(strtolower($rows['point_column']),$arr_add)){ //让球，大小，半场让球，半场大小，球队得分可赢金额要加上本金
                $rows['win']	+=	$rows['bet_money'];
            }
            $all_bet_money += $rows["bet_money"];
            $all_win += $rows['win'];
    ...
    *已结算 之 撤销结算
    ...
    public function do_re_jiesuan(){
        ...
        $db = Db::connect(config('sportdb'));        
        $update_data = 'match_js = 0,mb_inball=null,tg_inball=null';
        if($table=='bet_match'){
            $update_data .= ',match_sbjs = 0,mb_inball_hr=null,tg_inball_hr=null';
        }
        $db->query("update {$table} set {$update_data} where match_id in($mid)");
    ...
    private function back_page($table){
        ...
        $pages['badminton_match'] = url('sports/badminton');
        $pages['billiards_match'] = url('sports/billiards');
        $pages['other_match'] = url('sports/other');
        return $pages[$table];
    }
    ...
    private function ball_name($table){
        ...
        $names['badminton_match'] = '羽毛球';
        $names['billiards_match'] = '台球';
        $names['other_match'] = '其他';
        return $names[$table];
    }    
}