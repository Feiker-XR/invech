...
class User extends Login{
    ...
    public function set_card(){ //设置收款信息
    	...
    	$user = Db::table('k_user')->where(array('uid'=>$uid))->find();
    	if($_POST){
    		$paycard = trim($_POST['pay_card']);
    		$paynum= trim($_POST['pay_num']);
    		...
    		$address_4 = trim($_POST['Address_4']);
    		$address = $address_1.$address_2.$address_3.$address_4;
    		$moneypass = trim($_POST['MoneyPass']);
    ...
    public function userinfo(){ //额度转换
    	...
    	if (mb_substr($username,-3,3,'utf-8') != 'jsa') {
    	    $temp_username = $username . 'jsa';
    	} else {
    	    $temp_username = $username;
    	}
    	$mg_username = $username.'js';
    	...
    	if(request()->isGet()){
            ...
            if(request()->isMobile()){
                ...
                $balance_bb = $balance_bb['money'];
                $balance_og = $balance_og['money'];
                $balance_ag = $balance_ag['money'];
                $balance_mg = $balance_mg['money'];
                //$balance_sb = $balance_sb['money'];
                $balance_pt = $balance_pt['money'];
                /*
                $balance_bb = '0.00';
                $balance_og = '0.00';
                $balance_ag = '0.00';
	        ...
	        if ($type=="IN" || $type=="OUT"){
                ...
                if ($type=="IN") {
                    ...
                    //AG
                        ...
                        $type="AG";
                        $billno = "JSA{$uid}AG".time().rand(10,99);
    ...
    public function data_t_money(){//提现记录
    	...
    	$money = Db::table('k_money')->where($where)->order('m_id desc')->paginate(2,$num);
    	$page = $money->render();
    ...
    public function record_cp(){ //游戏记录  彩票游戏
    	$uid = Session::get('uid');
    	$user = Db::table('k_user')->where(array('uid'=>$uid))->find();
    	$cbet = Db::table('c_bet')->where(array('username'=>$user['username']))->order('addtime desc')->select();
