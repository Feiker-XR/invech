...
class pay extends Base {    //如果继承Login会导致无法充值回调无法通知到notify（）
    ...
    */
    ...
    public function member(){
        ...
        $huikuan = db('huikuan_bank')->where(['card_group'=>$gid,'card_qrcode'=>''])->order('id','desc')->where('card_ID','neq', '')->find();

        $alipay = db('huikuan_bank')->where(['card_group'=>$gid,'card_bankName'=>['like','%支付宝%']])->find();
        $wechat = db('huikuan_bank')->where(['card_group'=>$gid,'card_bankName'=>['like','%微信%']])->find();
        $qq = db('huikuan_bank')->where(['card_group'=>$gid,'card_bankName'=>['like','%QQ%']])->find();
