...
$url = "http://168kai.com/Open/CurrentOpen?code=10022";
ob_start();
$curl = curl_init();
...
foreach ($data['list'] as $k => $v) {
	...
	$time = substr(trim($qishu),0,4)."/".substr(trim($qishu),4,2)."/".substr(trim($qishu),6,2)." ".trim(substr(trim($v['c_d']),strrpos(trim($v['c_d']), ']')+1));
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
  <tr> 
    <td align="left">
      ...
      新疆时时彩(<?=$qishu?>期<?="$num1,$num2,$num3,$num4,$num5"?>):
      <span id="timeinfo"></span>
      </td>
  </tr>
</table>
<iframe src="../../Lottery/Auto/xsc.php?qi=<?=$jiesuan?>" frameborder="0" scrolling="no" height="0" width="0"></iframe>
</body>
</html>