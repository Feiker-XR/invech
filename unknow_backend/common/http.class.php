...
class cHTTP {
		...
		{
		  $this->curl = new Curl_HTTP_Client(false);
		  $cookie_path = dirname(__FILE__).'/../caiji/sport/cache/cookies.txt';
		  $this->curl->store_cookies($cookie_path);
		  //$this->curl->set_user_agent("Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.102 Safari/537.36");
		...
		function getContent(){
			...
			//$r=$this->curl->fetch_url($this->fetch_url);
			//$this->curl->close();
		...
		{
				return $this->curl->get_error_msg();
		}		
}
...
class Curl_HTTP_Client
{
	...
	function set_proxy_username_password($username,$password)
	{
		//设置通过的HTTP代理服务器， 代理的用户名和密码
