...
include_once("../../member/cache/zqgq.php");
include_once("../include/pd_user_json.php");
if(time()-$lasttime > 3){ //超时
    echo __LINE__;
	if($count == 0){ //没数据
		$json["dh"]	=	0;
		$json["fy"]["p_page"] = "0";
		...
		include_once("../../member/include/function_cj.php");
		if(zqgq_cj()){
			include("../../member/cache/zqgq.php"); //重新载入
		}else{
		    echo __LINE__;
			$json["dh"]	=	0;
			$json["fy"]["p_page"] = "0";
...
if($count_num == "0"){
	...
	for($b=$start; $b<=$end; $b++){ 
		...
		$json["db"][$i]["Match_DxGG"]		=	$rows[$b]["Match_DxGG"] ? "大".$rows[$b]["Match_DxGG"] : "";     	// 17
		$json["db"][$i]["Match_Bdxpk"]		=	$rows[$b]["Match_Bdxpk"] ? "大".$rows[$b]["Match_Bdxpk"] : "";      //   18
		$json["db"][$i]["Match_DxGG1"]		=	$rows[$b]["Match_DxGG"] ? "小".$rows[$b]["Match_DxGG"] : "";        //  19
		$json["db"][$i]["Match_Bdxpk2"]		=	$rows[$b]["Match_Bdxpk"] ? "小".$rows[$b]["Match_Bdxpk"] : "";     	//  20
