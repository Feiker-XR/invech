...
$sql		=	"select id from bet_match where Match_Type=1 and match_date='".date("m-d")."' AND Match_CoverDate>'".date('Y-m-d H:i:s')."' and (Match_BHo+Match_BAo<>0 or Match_Bdpl+Match_Bxpl<>0) and Match_IsShowb=1 ".$sqlwhere.' order by Match_CoverDate,iPage,match_name,Match_Master,Match_ID,iSn';
...
if($i == 1){ //未查找到记录
	...
	while($rows = $query->fetch_array()){ 
		...
		$json["db"][$i]["Match_Bdxpk1"]		=	"大".$rows["Match_Bdxpk"];     ///////////     15
		$json["db"][$i]["Match_Hr_ShowType"]=	$rows["Match_Hr_ShowType"];     ///////////     16
		$json["db"][$i]["Match_Bdxpk2"]		=	"小".$rows["Match_Bdxpk"];     ///////////     17
		$i++;
	}
}
