    ...
    $.post("class/odds_3.php", function(data)
        {
            if(data.opentime>0){
				...
				parent.$.jBox.alert('广东快乐十分已经封盘，请稍后再进行下注！', '提示');
				return false;
            }
        }, "json");
}
...
function endtime(iTime)
{
    ...
    if(iTime==0){
        ...
        $('.kick').each(function(){
            ...
            e.prop('src','images/Ball_3/x.png');
            i++;
        });
    }
    if(iTime==60){
    ...
    $.post("class/auto_3.php", {ball : ball}, function(data)
        {
            ...
            var auto_top = '<table width="100%" border="0" cellspacing="1" cellpadding="0" class="clbian"><tr class="clbian_tr_title"><td colspan="2">开奖结果</td></tr><tr class="clbian_tr_title"><td>开奖期数</td><td>开奖号码</td></tr>';
                for (var key in data.hmlist){
                    auto_top = auto_top+'<tr class="clbian_tr_txt"><td class="qihao">'+key+'</td><td class="haoma">'+BuLing(data.hmlist[key])+'</td></tr>'
                }
