...
function loaded(league,thispage,p){
	...
	$.getJSON("bk_danjie_data.php?leaguename="+league+"&CurrPage="+thispage+"&callback=?",function(json){
			...
			for(var i=0; i<dbs.length; i++){
				...
				htmls+="<div class=\"bisai_bo\">";
					...
					htmls+="<div class=\"zhudui_bo\">";
						...
						htmls+="<div class=\"hui_z zu_4_2\">"+(dbs[i]["Match_DsDpl"]<="0" ? "" :("<a href=\"javascript:void(0)\" title=\"单\" onclick=\"javascript:setbet('篮球单式','单双-单','"+dbs[i]["Match_ID"]+"','Match_DsDpl','0','0','单');\" style='"+(dbs[i]["Match_DsDpl"]!=data[i]["Match_DsDpl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_DsDpl"],2)+"</a>"))+"</div>";
						htmls+="</div>";
					htmls+="</div>";
					htmls+="<div class=\"kedui_bo\">";
						...
						htmls+="<div class=\"hui_z zu_4_2\">"+(dbs[i]["Match_Ao"]<="0" ? "" :("<a href=\"javascript:void(0)\" title=\""+dbs[i]["Match_Guest"]+"\" onclick=\"javascript:setbet('篮球单式','让球-"+(dbs[i]["Match_ShowType"]=="H"?"主让":"客让")+dbs[i]["Match_RGG"]+"-"+dbs[i]["Match_Guest"]+"','"+dbs[i]["Match_ID"]+"','Match_Ao','1','0','"+dbs[i]["Match_Guest"]+"');\" style='"+(dbs[i]["Match_Ao"]!=data[i]["Match_Ao"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_Ao"],2)+"</a>"))+"</div>";
						htmls+="</div>";
						htmls+="<div class=\"bodan_x zu_4\">";
						...
						htmls+="<div class=\"hui_z zu_4_2\">"+(dbs[i]["Match_DsDpl"]<="0" ? "" :("<a href=\"javascript:void(0)\" title=\"双\" onclick=\"javascript:setbet('篮球单式','单双-双','"+dbs[i]["Match_ID"]+"','Match_DsSpl','0','0','双');\" style='"+(dbs[i]["Match_DsDpl"]!=data[i]["Match_DsDpl"] && data[i]["Match_ID"]==dbs[i]["Match_ID"]?"background:#FFCC00":"")+"'>"+formatNumber(dbs[i]["Match_DsSpl"],2)+"</a>"))+"</div>";
						htmls+="</div>";
					htmls+="</div>";
				htmls+="</div>";
				htmls+="</div>";
